...
float top_pt_reweight(float top_pt, int top_type) {
  float ret = 1.0;
  if (top_type == 0) {
    if (top_pt < 375.0 ){
      ret = (7.59e-01 + (3.52e-04 * top_pt) + (3.11e-01 * exp(-4.24e-03 * top_pt)) );
    } else {
      ret = (1.04e+00 + (-2.31e-04 * top_pt) + (-2.97e+04 * exp(-7.43e-01 * top_pt)) );
    }
  } else if (top_type == 1) {
  	ret = (9.73e-01 + (-1.34e-04 * top_pt) + (1.03e-01 * exp(-1.18e-02 * top_pt)) );
  }
  return ret;
}
    ...
    if(isTTbar)
    {
      for(unsigned iMC=0 ; iMC<LHE_Pt->size() ; ++iMC)
      {
        if( (abs(LHE_pdgid->at(iMC)) == 6) ) {
          // top_type 0: POWHEG P8 (T4), 1: POWHEG P8 (CP5)
          w_top = w_top * top_pt_reweight(LHE_Pt->at(iMC), 1);
