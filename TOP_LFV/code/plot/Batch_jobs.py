...
parser.add_option("-n","--n_per_job",dest="n_per_job",default=-1,type="int")
    ...
    for sys_type in Sys_sample_plot_dic:
        ...
        for sample in Sample_plot_dic:
            ...
            if options.sample_name !="null":
                if not (sample in options_sample_name_list):continue
...
options_sample_name_list = []
if options.sample_name != "null":
    options_sample_name_list = options.sample_name.split(",")
    print "Sample list: %s "%(options_sample_name_list)

for sys_type in sys_dic:
    if GLOBAL_USE_SYS or (sys_type == "nominal"):
...
if options.n_per_job > 0:
    n_per_job_input = options.n_per_job
else:
    n_per_job_input = 500000
print "%s / job"%(n_per_job_input)
make_sub("split_plot_80",n_per_job_input)
