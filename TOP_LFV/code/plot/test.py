...
def main_plot_part(input_class, path, plot_name):
	...
	#sys case
	if GLOBAL_USE_SYS:
		for tmp_sys_type in global_sys_class_dic:
			if tmp_sys_type == "nominal":continue
			global_sys_class_dic[tmp_sys_type].get_sys_sum(path)
		sys_value_dic = get_sys_list_dic(path, global_sys_class_dic, compare2_sys)
		hist_set_sys(compare2_sys, sys_value_dic)
		if path == "n_stat" and options.log_level > 0:
			print_sys_contribution(sys_value_dic)
		if path == "M_emu_massDep" and options.log_level > 0:
	...
	compare2_sum_todraw = compare2_sum.Clone("tmp_tt")
	compare2_sum_todraw.SetFillStyle(3015)
	compare2_sum_todraw.SetFillColor(ROOT.kOrange-3)
	if GLOBAL_USE_SYS:
		for i in range(1, compare2_sum.GetNbinsX() +1):
			stat_error_i = compare2_sum.GetBinError(i)
			sys_error_i = compare2_sys.GetBinError(i)
			fianl_error_i = sqrt(stat_error_i*stat_error_i + sys_error_i*sys_error_i)
			compare2_sum_todraw.SetBinError(i, fianl_error_i)
	...
	compare2_sum_todraw.Draw("same:E2")
	gr_1.Draw("PE")
	if GLOBAL_USE_SYS:
		legend1.AddEntry(compare2_sum_todraw,"Uncertainties","f")
	else:
		legend1.AddEntry(compare2_sum_todraw,"Stat. uncertainty","f")
...
#	legend3.Draw()	
	...
	pad2.Draw()
	pad2.cd()	
	h_ratio = compare2_sum_todraw.Clone(h_ratio.GetName())
	...
	h_ratio.Draw("E2")
	gr_ratio = input_class.get_graph_ratio(input_class.histTograph(compare1_sum, input_class.value_dic[path]["use_array"]), ROOT.TGraphAsymmErrors(compare2_sum_todraw))
...
def init_class(input_class, input_sys_type, set_dir = True):
	...
	input_class.log_level = options.log_level

def Store_limit_plot1(plot_name, sys_class_dic):
	name_dic={}
	...
	for path in sys_class_dic["nominal"].value_dic:
		if not (path in sys_class_dic["nominal"].store_hist_plot_list):continue
		...
		for tmp_sys_type in sys_class_dic:
			...
			for part in tmp_class.plot_dic["compare_3"]:
				...
				if "_" == process[0]:
					process = process[1:]
				if "RPVresonantToEMu" in process:
					process = process.replace("RPVresonantToEMu", "RPVToEMu")
				systematic = sys_dic[tmp_sys_type][0]
				tmp_hist.SetName("%s_%s"%(process, systematic))
				tmp_hist.Write()
				tmp_hist.SetName(pre_name)
...
def Store_limit_plot2(plot_name, sys_class_dic):
	name_dic={}
	name_dic["_ttbar"] = "TTbar"
	name_dic["Z"] = "DY"
	name_dic["ST"] = "ST"
	...
	for path in sys_class_dic["nominal"].value_dic:
		...
		for part in sys_class_dic["nominal"].plot_dic["compare_1"]:
			for tmp_sys_type in sys_class_dic:
				tmp_class = sys_class_dic[tmp_sys_type]
				if not "compare_1" in tmp_class.plot_dic:continue
				if part in tmp_class.plot_dic["compare_1"]:
					tmp_hist = tmp_class.plot_dic["compare_1"][part]["hist"][path]
					pre_name = tmp_hist.GetName()
					process = name_dic[part]
					systematic = sys_dic[tmp_sys_type][0]
		...
		for part in sys_class_dic["nominal"].plot_dic["compare_2"]:
			for tmp_sys_type in sys_class_dic:
				tmp_class = sys_class_dic[tmp_sys_type]
				if not "compare_2" in tmp_class.plot_dic:continue
				if part in tmp_class.plot_dic["compare_2"]:
					tmp_hist = tmp_class.plot_dic["compare_2"][part]["hist"][path]
					pre_name = tmp_hist.GetName()
					process = name_dic[part]
					systematic = sys_dic[tmp_sys_type][0]
		...
		for part in sys_class_dic["nominal"].plot_dic["compare_3"]:
			for tmp_sys_type in sys_class_dic:
				tmp_class = sys_class_dic[tmp_sys_type]
				if not "compare_3" in tmp_class.plot_dic:continue
				if part in tmp_class.plot_dic["compare_3"]:
					tmp_hist = tmp_class.plot_dic["compare_3"][part]["hist"][path]
					pre_name = tmp_hist.GetName()
					process = part
					if "_" == process[0]:
...
def main_plot(input_sys_type):
	global GLOBAL_USE_SYS
	...
	if GLOBAL_USE_SYS:
		for tmp_sys_type in sys_dic:
			if tmp_sys_type == "nominal":continue
			global_sys_class_dic[tmp_sys_type] = hist_make(GLOBAL_YEAR)
			init_class(global_sys_class_dic[tmp_sys_type], tmp_sys_type)
			global_sys_class_dic[tmp_sys_type].total_lumi = p_nominal.Get_total_lumi(p_nominal.input_dic)
			global_sys_class_dic[tmp_sys_type].main_plot()
			global_sys_class_dic[tmp_sys_type].set_common_hist("qcd_jet", p_nominal.plot_dic["compare_2"]["qcd_jet"])
			if (sys_dic[tmp_sys_type][1]):
				global_sys_class_dic[tmp_sys_type].Store_hist_plot_total("23")
			else:
				global_sys_class_dic[tmp_sys_type].Store_hist_plot_total("2")
	...
	for tmp_sys_type in global_sys_class_dic:
		global_sys_class_dic[tmp_sys_type].close()
	return
...
#	#~~~~~~~~~~~~~~same sign
	...
	GLOBAL_USE_SYS = pre_GLOBAL_USE_SYS


def main_plot_split(input_sys_type):
	p_nominal_split = hist_make(GLOBAL_YEAR)
	init_class(p_nominal_split, input_sys_type)
	p_nominal_split.main_plot_split()
	p_nominal_split.close()

def print_table(mc_list, data_str):
	title_row = "Samples" + ";"
	mc_row_dic = collections.OrderedDict()
	for mc_name in mc_list:
		mc_row_dic[mc_name] = mc_name + ";"
	...
	for cut_str_tmp in global_cutflow_table:
		...
		for mc_name in mc_row_dic:
			...
			mc_row_dic[mc_name] += ("%0.1f"%(mc_nevent) + ";" )
			mc_total += mc_nevent
		prediction_row += ("%0.1f"%(mc_total) + ";" )
		data_total = global_cutflow_table[cut_str_tmp][data_str][0]
		data_row += ("%0.1f"%(data_total) + ";" )
		if mc_total == 0:
			ratio_row += ("--" + ";")
		else:
			ratio_row += ("%0.1f"%(data_total/mc_total) + ";")
	file_cut_table = open("cut_flow_table_%s.txt"%(GLOBAL_YEAR), "w" )
	file_cut_table.write("%s\n"%(title_row) )
	for mc_name in mc_list:
		file_cut_table.write("%s\n"%(mc_row_dic[mc_name]))
	file_cut_table.write("%s\n"%(prediction_row) )
	file_cut_table.write("%s\n"%(data_row) )
