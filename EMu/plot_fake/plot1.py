...
def main_plot_part_fk(path, input_dic, plot_dic, cut, c1):
	pad1=ROOT.TPad("pad1", "pad1", 0, 0.315, 1, 0.99 , 0)#used for ratio, the hist plot
	pad2=ROOT.TPad("pad2", "pad2", 0, 0.0, 1, 0.3 , 0)#used for ratio, the ratio plot
	pad1.SetBottomMargin(0.0)
	pad1.SetLeftMargin(0.14)
	...
	#c1.cd()
	c1.Update()
	c1.Print("_FK%s/FK_%s_hratio_%s.png"%(cut,cut[1:],path))
	c1.Print("_FK%s/FK_%s_hratio_%s.pdf"%(cut,cut[1:],path))
...
def Init_hist(input_dic, value_dic, plot_dic):
	for path in value_dic:
		if value_dic[path]["use_array"]:
			...
			for part in plot_dic_fk:
				for plot in plot_dic_fk[part]:
					plot_dic_fk[part][plot]["hist"][path] = ROOT.TH1F("%s_%s"%(sample,value_dic[path]["hist_name"]), "", value_dic[path]["hist_para"][0], value_dic[path]["hist_para"][1])
		else:
			for sample in input_dic:
			...
			for part in plot_dic_fk:
				for plot in plot_dic_fk[part]:
					plot_dic_fk[part][plot]["hist"][path] = ROOT.TH1F("%s_%s"%(sample,value_dic[path]["hist_name"]), "", value_dic[path]["hist_para"][0], value_dic[path]["hist_para"][2], value_dic[path]["hist_para"][3])
...
def main_plot(input_dic, value_dic, plot_dic, cut, isPlot = True):
	...
	Set_hist_fk(input_dic, value_dic, plot_dic_fk)
	Get_part_hist_fk(input_dic, value_dic, plot_dic_fk)
	c1 = ROOT.TCanvas( 'c1', 'A Simple Graph with error bars', 50,50,865,780 )
	c1.SetLeftMargin(0.5)

	for path in value_dic:
		if path == "mass_err_stat":continue
		main_plot_part_fk(path, input_dic, plot_dic_fk, cut, c1)
	c1.Close()
...
"compare_2":{
			...
			"fake_rate":{
					"data_list":['fk_data','fk_ttbar_1','fk_ttbar_2','fk_ttbar_3','fk_ttbar_4','fk_ttbar_5','fk_ST','fk_WW1','fk_WW2','fk_WW3','fk_WW4','fk_WW5','fk_WZ','fk_ZZ','fk_DYToLL_1','fk_DYToLL_2','fk_DYToLL_3','fk_DYToLL_4','fk_DYToLL_5','fk_DYToLL_6','fk_DYToLL_7','fk_DYToLL_8','fk_DYToLL_9'],
					"color":ROOT.kYellow,
					"legend_title":"Jet fake",
					"N_total":0.0,
					"hist":{},
					"ABS":True,
					},
...
"compare_2":{
			"_ttbar":{
					"data_list":["fk_ttbar_1","fk_ttbar_2","fk_ttbar_3","fk_ttbar_4","fk_ttbar_5"],
					"color":ROOT.kRed-4,
					"legend_title":"t#bar{t}",
					"N_total":0.0,
					"hist":{},
...
plot_list = ["_ttbar","Di_boson","ST","Z","fake_rate"]
