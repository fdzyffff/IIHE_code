...
def Fill_hist(sample_dic, value_dic, cut):
	...
	for event in t1:
		...
		if (not isFake) and (getattr(event,"ele_charge") != getattr(event,"muon_charge")):continue
		event_weight = 1.0
		MuonSF_weight = 1.0
...
def main_plot_part_fk(path, input_dic, plot_dic, cut, c1):
	...
	tText_5.SetTextFont(40)
	tText_5.SetTextSize(0.03)
	tText_5.AddText ('same sign %s'%(cut_name[cut][1]))
	tText_5.Draw()
	...
	#c1.cd()
	c1.Update()
	c1.Print("_SS%s/SS_%s_hratio_%s.png"%(cut,cut[1:],path))
	c1.Print("_SS%s/SS_%s_hratio_%s.pdf"%(cut,cut[1:],path))
	...
	for path in value_dic:
		for part in plot_dic_fk:
			for plot in plot_dic_fk[part]:
				...
				plot_dic_fk[part][plot]["hist"][path].SetFillColorAlpha(plot_dic_fk[part][plot]["color"],1.0)
				plot_dic_fk[part][plot]["hist"][path].SetLineColorAlpha(plot_dic_fk[part][plot]["color"],1.0)
...
plot_dic={
"compare_1":{"data":{
					"data_list":['data','_ttbar_1','_ttbar_2','_ttbar_3','_ttbar_4','_ttbar_5','_ST','_WW1','_WW2','_WW3','_WW4','_WW5','_WZ','_ZZ','_DYToLL_1','_DYToLL_2','_DYToLL_3','_DYToLL_4','_DYToLL_5','_DYToLL_6','_DYToLL_7','_DYToLL_8','_DYToLL_9'],
					"color":1,
...
'_MET_0to30':'getattr(event,"MET_T1Txy_Pt") <= 30',
