...
class hist_make():
	...
	def init_2016(self):
		...
		self.MuonISOSF_dic			= self.Load_json("SF_json/SF_json_2016/Muon/Iso/Muon_2016_SF_ISO.json")
		self.MuonIDSF_dic			= self.Load_json("SF_json/SF_json_2016/Muon/ID/Muon_2016_SF_ID.json")

		#self.MuonTriggerSF_sysu_dic		= self.Load_json("SF_json/SF_json_2016/Muon/Trigger/sysu_Muon_Trigger_2018_SF.json")
		self.MuonISOSF_sysu_dic			= self.Load_json("SF_json/SF_json_2016/Muon/Iso/sysu_Muon_2016_SF_ISO.json")
		self.MuonIDSF_sysu_dic			= self.Load_json("SF_json/SF_json_2016/Muon/ID/sysu_Muon_2016_SF_ID.json")
	
		#self.MuonTriggerSF_sysd_dic		= self.Load_json("SF_json/SF_json_2016/Muon/Trigger/sysd_Muon_Trigger_2018_SF.json")
		self.MuonISOSF_sysd_dic			= self.Load_json("SF_json/SF_json_2016/Muon/Iso/sysd_Muon_2016_SF_ISO.json")
		self.MuonIDSF_sysd_dic			= self.Load_json("SF_json/SF_json_2016/Muon/ID/sysd_Muon_2016_SF_ID.json")


		self.dic_Ele_reco_pt_eta	= self.Load_json("SF_json/SF_json_2016/Electron/reco/Electron_2016_SF_reco.json")
	...
	def init_2017(self):
		self.MuonTriggerSF_dic		= self.Load_json("SF_json/SF_json_2017/Muon/Trigger/Muon_Trigger_runBCDEF_SF.json")
		self.MuonISOSF_dic			= self.Load_json("SF_json/SF_json_2017/Muon/Iso/Muon_ISO_RunBCDEF_SF_ISO.json")
		self.MuonIDSF_dic			= self.Load_json("SF_json/SF_json_2017/Muon/ID/Muon_ID_RunBCDEF_SF_ID.json")

		self.MuonTriggerSF_sysu_dic		= self.Load_json("SF_json/SF_json_2017/Muon/Trigger/sysu_Muon_Trigger_runBCDEF_SF.json")
		self.MuonISOSF_sysu_dic			= self.Load_json("SF_json/SF_json_2017/Muon/Iso/sysu_Muon_ISO_RunBCDEF_SF_ISO.json")
		self.MuonIDSF_sysu_dic			= self.Load_json("SF_json/SF_json_2017/Muon/ID/sysu_Muon_ID_RunBCDEF_SF_ID.json")
	
		self.MuonTriggerSF_sysd_dic		= self.Load_json("SF_json/SF_json_2017/Muon/Trigger/sysd_Muon_Trigger_runBCDEF_SF.json")
		self.MuonISOSF_sysd_dic			= self.Load_json("SF_json/SF_json_2017/Muon/Iso/sysd_Muon_ISO_RunBCDEF_SF_ISO.json")
		self.MuonIDSF_sysd_dic			= self.Load_json("SF_json/SF_json_2017/Muon/ID/sysd_Muon_ID_RunBCDEF_SF_ID.json")


		self.dic_Ele_reco_pt_eta	= self.Load_json("SF_json/SF_json_2017/Electron/reco/Electron_reco_run2017BCDEF_json.txt")
		self.ElectronIDSF_dic = {
				(0,1.4442):0.963,
				(1.566,2.5):0.967,
			}

		self.dic_ttbar_gen_mass		= self.Load_json("SF_json/SF_json_2017/ttbar/ttbar_NNPDF31to30_json.txt")
	...
	def check_trigger_2016(self,leaf_name_list,event):
		...
		if (Muon50_trig_fire) and ("Muon50_trig_fire" in leaf_name_list) and  getattr(event,"Muon50_trig_fire"):
			pass_trigger = True
			pass_trigger_onlyMuon = True
		if (TkMu50_trig_fire) and ("TkMu50_trig_fire" in leaf_name_list) and  getattr(event,"TkMu50_trig_fire"):
			pass_trigger = True
			pass_trigger_onlyMuon = True
	...
	def get_event_weight_2016(self, event):
		...
		ElectronSF_weight *= self.Get_SF_2D(self.dic_Ele_reco_pt_eta,getattr(event,"ele_Et"),getattr(event,"ele_eta"))
		...
		#sys muon id sf
		if self.sys_type == "mu_id_sfu":
			MuonSF_weight *= self.Get_SF_2D(self.MuonIDSF_sysu_dic,getattr(event,"muon_pt"),abs(getattr(event,"muon_eta")))
		elif self.sys_type == "mu_id_sfd":
			MuonSF_weight *= self.Get_SF_2D(self.MuonIDSF_sysd_dic,getattr(event,"muon_pt"),abs(getattr(event,"muon_eta")))
		else :
			MuonSF_weight *= self.Get_SF_2D(self.MuonIDSF_dic,getattr(event,"muon_pt"),abs(getattr(event,"muon_eta")))

		#sys muon iso sf
		if self.sys_type == "mu_iso_sfu":
			MuonSF_weight *= self.Get_SF_2D(self.MuonISOSF_sysu_dic,getattr(event,"muon_pt"),abs(getattr(event,"muon_eta")))
		elif self.sys_type == "mu_iso_sfd":
			MuonSF_weight *= self.Get_SF_2D(self.MuonISOSF_sysd_dic,getattr(event,"muon_pt"),abs(getattr(event,"muon_eta")))
		else :
			MuonSF_weight *= self.Get_SF_2D(self.MuonISOSF_dic,getattr(event,"muon_pt"),abs(getattr(event,"muon_eta")))

		#sys muon iso sf
		if pass_trigger_onlyMuon:
			pt = getattr(event,"muon_pt")
			if self.sys_type == "mu_trig_sfu":
				if pt < 300:
					MuonSF_weight *= 1 + sqrt(0.02 * 0.02 + 0.02 * 0.02)
			elif self.sys_type == "mu_trig_sfd":
				if pt < 300:
					MuonSF_weight *= 1 - sqrt(0.02 * 0.02 + 0.02 * 0.02)

		#sys electron id sf
		if self.sys_type == "el_id_sfu":
			ElectronSF_weight *= self.Get_SF_1D(self.ElectronIDSF_dic,abs(getattr(event,"ele_eta"))) * (1.0 + self.Get_HEEP_ID_sys(getattr(event,"ele_Et"), getattr(event,"ele_region")) )
		elif self.sys_type == "el_id_sfd":
			ElectronSF_weight *= self.Get_SF_1D(self.ElectronIDSF_dic,abs(getattr(event,"ele_eta"))) * (1.0 - self.Get_HEEP_ID_sys(getattr(event,"ele_Et"), getattr(event,"ele_region")) )
		else :
			ElectronSF_weight *= self.Get_SF_1D(self.ElectronIDSF_dic,abs(getattr(event,"ele_eta")))

		return MuonSF_weight * ElectronSF_weight * top_weight * prefiring_weight

	def get_event_weight_2017(self, event):
		MuonSF_weight = 1.0
		ElectronSF_weight = 1.0
		top_weight = 1.0
		prefiring_weight = 1.0
		ttbarNNPDF_weight = 1.0
		...
		if self.sys_type == "top1_u":
			...
			top_weight = getattr(event,"w_top")
		if (top_weight != 1.0):	ttbarNNPDF_weight *= self.Get_SF_1D(self.dic_ttbar_gen_mass, getattr(event,"gen_mass"))
		...
		#sys muon iso sf
		if pass_trigger_MuonOnly:#getattr(event,"Muon50_trig_fire"):
			if self.sys_type == "mu_trig_sfu":
				MuonSF_weight *= self.Get_SF_2D(self.MuonTriggerSF_sysu_dic,getattr(event,"muon_pt"),abs(getattr(event,"muon_eta")))
			elif self.sys_type == "mu_trig_sfd":
				MuonSF_weight *= self.Get_SF_2D(self.MuonTriggerSF_sysd_dic,getattr(event,"muon_pt"),abs(getattr(event,"muon_eta")))
			else :
				MuonSF_weight *= self.Get_SF_2D(self.MuonTriggerSF_dic,getattr(event,"muon_pt"),abs(getattr(event,"muon_eta")))
		...
		return MuonSF_weight * ElectronSF_weight * top_weight * prefiring_weight * ttbarNNPDF_weight

	def get_event_weight_2018(self, event):
		MuonSF_weight = 1.0
		ElectronSF_weight = 1.0
		top_weight = 1.0
		prefiring_weight = 1.0
	...
	def Fill_hist(self, sample_dic, value_dic, cut_str):
		...
		for event in t1:
			...
			for path in value_dic:
				...
				event_weight *= pu_weight * fake_weight
...
#			tmp_sum.Add(tmp_sum, common_sys, 1, 1)
	...
	def set_common_hist(self, plot, tmp_common_dic):
		...
		for path in tmp_common_dic["hist"]:
			...
			if self.sys_type == "lumi_u":
				self.plot_dic[part][plot]["hist"][path].Scale(1.0 + self.lumi_sys)
			if self.sys_type == "lumi_d":
				self.plot_dic[part][plot]["hist"][path].Scale(1.0 - self.lumi_sys)
