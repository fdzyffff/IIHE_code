...
def Getweight(fnamedic_data,fnamedic_mc):
    ...
    for (fname_in,fname_out) in fnamedic_mc:
        ...
        if isUpdate:
            if 'fk' in fname_out:
                if isUpdate_fk:
                    fnamedic_mc[(fname_in,fname_out)][1][0]*=fnamedic_mc[(fname_in,fname_out)][0][0]
            else:
                fnamedic_mc[(fname_in,fname_out)][1][0]*=fnamedic_mc[(fname_in,fname_out)][0][0]
        print '%-25s %-8.2f\t%-8.2f\t%-8.2f\t%0.2f %%'%(fname_in,fnamedic_mc[(fname_in,fname_out)][0][0],fnamedic_mc[(fname_in,fname_out)][0][1],fnamedic_mc[(fname_in,fname_out)][1][0]*fnamedic_mc[(fname_in,fname_out)][4][0],float(fnamedic_mc[(fname_in,fname_out)][1][0])/ndata*100)
        n+=fnamedic_mc[(fname_in,fname_out)][1][0]*fnamedic_mc[(fname_in,fname_out)][4][0]
        if not isUpdate:
    ...
    for (fname_in,fname_out) in fnamedic_mc:
        ...
        if isUpdate:
            if 'fk' in fname_out:
                if isUpdate_fk:
                    fnamedic_mc[(fname_in,fname_out)][1][1]*=fnamedic_mc[(fname_in,fname_out)][0][0]
            else:
                fnamedic_mc[(fname_in,fname_out)][1][1]*=fnamedic_mc[(fname_in,fname_out)][0][0]
        print '%-25s %-8.2f\t%-8.2f\t%-8.2f\t%0.2f %%'%(fname_in,fnamedic_mc[(fname_in,fname_out)][0][0],fnamedic_mc[(fname_in,fname_out)][0][1],fnamedic_mc[(fname_in,fname_out)][1][1]*fnamedic_mc[(fname_in,fname_out)][4][0],float(fnamedic_mc[(fname_in,fname_out)][1][1])/ndata*100)
        n+=fnamedic_mc[(fname_in,fname_out)][1][1]*fnamedic_mc[(fname_in,fname_out)][4][0]
        if not isUpdate:
            fnamedic_mc[(fname_in,fname_out)][0][0]=1
...
def my_plot(fname_in,fname_out,plotlistdic,filterstring,isData,isFake=False):
    ...
    for path in plotlistdic:
        ...
        exec "h_%s.Sumw2()" % (path+fname_out)
    nevent_process = 0
    nevent_filed = 0.0
    nevent_filed_PU = 0.0
...
#        if nevent_process>1000:break
        ...
        if isData :
            if isFake:
                nevent_filed += getattr(event,"w_PU_combined")*sf_weight*getattr(event,"fake_weight")
            else:
                nevent_filed += getattr(event,"w_PU_combined")*sf_weight
        elif isFake :
            nevent_filed += pm(getattr(event,"w_PU_combined"))*sf_weight*getattr(event,"fake_weight")
            nevent_filed_PU += getattr(event,"w_PU_combined")*sf_weight*getattr(event,"fake_weight")
        else:
    ...
    if isData:
        ...
        print fnamedic_mc[(fname_in,fname_out)][1][0]
        print fnamedic_mc[(fname_in,fname_out)][1][1]

                
def my_get(fname_in,fname_out,plotlistdic,isData,tmp_hfile):
    for path in plotlistdic:
        exec "h_%s = tmp_hfile.Get('%s')" % (path+fname_out,plotlistdic[path][0][1]+fname_out)
    if isData:
        for path in plotlistdic:
...
def main_plot(fnamedic_part1,fnamedic_part2,plotlistdic,isUpdate,filterstring,label):
    ...
    if isUpdate_fk:
        hfile_fk = ROOT.TFile(label+'/kkk_fk_'+label+'.root', 'RECREATE', 'Demo ROOT file with histograms' )
    else:
        hfile_fk = ROOT.TFile(label+'/kkk_fk_'+label+'.root', 'READ', 'Demo ROOT file with histograms' )
    ...
    for (fname_in,fname_out) in fnamedic_data:
        for path in plotlistdic:
            if isUpdate:
                ...
                #exec "my_setzero(h_%s)"% (path+fname_out)
                if not 'fk' in fname_out:
                    exec "h_%s = hfile.Get('%s')" % (path+fname_out,plotlistdic[path][0][1]+fname_out)    
            if not isUpdate_fk:
                if 'fk' in fname_out:
                    exec "h_%s = hfile_fk.Get('%s')" % (path+fname_out,plotlistdic[path][0][1]+fname_out)    
        ...
        if isUpdate:
            if 'fk' in fname_out:
                if isUpdate_fk:
                    ...
                    my_get(fname_in,fname_out,plotlistdic,True,hfile_fk)        
            else:
...
#            exec "my_setzero(h_%s)"% (path+fname_out)
    ...
    #add
    c1 = ROOT.TCanvas( 'c1', 'A Simple Graph with error bars', 800,640 )
    #c1.cd()
        ...
        for path in plotlistdic:
            ...
            for (fname_in_tmp, fname_out_tmp) in fnamedic_part2:
                if fname_out_tmp in tmp_name_list:
                    ...
                    exec "hst_part_%s.Add(hst_part_%s,h_%s,1,%s)" %(path+fname_out,path+fname_out,path+fname_out_tmp,fnamedic_part2[(fname_in_tmp,fname_out_tmp)][4][0])
            ...
            if fnamedic_part3[(fname_in,fname_out)][6][0] == 1:
                exec "my_abs(hst_part_%s) " % (path+fname_out)
...
###insert data from old root####

        tmp_fname_in = "MINIAOD_%s/kkk_%s.root"%(label, label)
        exec "hfile_outside = ROOT.TFile(tmp_fname_in,'READ','outside input root')"
        tmp_str = plotlistdic[path][0][1]+"data"
        print path
        exec "h_tmp_%s = hfile_outside.Get('%s')" % (path,tmp_str)
        exec "h_out_%s = h_tmp_%s.Clone('h_out_%s')" % (path,path,path)
        exec "h_out_%s.SetMarkerStyle(23)" % (path)
        exec "h_out_%s.SetLineColor(2)" % (path)
        exec "h_out_%s.SetMarkerColor(2)" % (path)
        exec "h_out_%s.SetStats(0)" % (path)
###
        try:   
        ...
        exec 'legend.AddEntry(h_out_%s,"%s","pl")' %(path,"MINIAOD")
...
#Text
        ...
        t1 = tText_3.AddText ("36.8 fb^{-1}, 13 TeV")
...
#        pad1.Print(label+'/hst_%s_%s.png'%(path,label))
        ...
        exec 'h_out_%s.Draw("sames:PE")' % (path)
#        pad1.Print(label+'/hsum_%s_%s.png'%(path,label))
#        pad1.SetBottomMargin(0.005)
        pad1.Draw()
    
        pad2.cd()
        exec "hratio_%s.Divide(h_out_%s)" % (path,path)
        exec "hratio_%s.SetMinimum(0.65)" % (path)
        exec "hratio_%s.SetMaximum(1.25)" % (path)
        ...
        exec "hratio_%s.GetYaxis().SetTitle('AOD / MINIAOD')" %   (path)
        exec "hratio_%s.GetYaxis().CenterTitle()" %   (path)
        exec "hratio_%s.GetYaxis().SetTitleSize(0.08)" %   (path)
        exec "hratio_%s.GetYaxis().SetTitleOffset(0.4)" %   (path)
        ...
        #c1.cd()
        c1.Update()
        c1.Print(label+'/com_hratio_%s_%s.png'%(path,label))
...
'heep1_Et':[[['t_Et'],'Et','distribution of heep1 Et ','30','x_pt','50','1500','0.00001','100000'],['E_{t}(e) (Gev/c)',0.04,'Event / Gev',0.05],[1,1,1],[1]],
'muon_Et':[[['muon_pt'],'muon_pt','distribution of heep2 Et','30','x_pt','50','1500','0.00001','100000'],['P_{t}(#mu) (Gev/c)',0.04,'Event / Gev',0.05],[1,1,1],[1]],
...
fnamedic_part1={
('data_2016_SingleMuon_SinglePhoton2.root','data'):[[1, 36814,0,0],[-1],[38],[1],[1]],
}

fnamedic_part2={
#('ttbar.root','_ttbar'):[[1,0, 87.31,100606883],  [-1,-1],[50],[0],[1]],
('ttbar2l2u_0-1.root','_ttbar'):[[1,0, 87.31,11645560],  [-1,-1],[50],[0],[1]],
('DYToLL.root','_DYToLL'):[[1,0, 5765.4,46380395],  [-1,-1],[5],[0],[1]],
#('WJets.root','_WJets'):[[1,0, 61526.7, 86006937],  [-1,-1],[8],[0],[1]],
('WW1.root','_WW1'):      [[1,0, 12.178,   1565922],    [-1,-1],[30],[0],[1]],
('WW2.root','_WW2'):      [[1,0, 0.1322,   200000],    [-1,-1],[30],[0],[1]],
('WW3.root','_WW3'):      [[1,0, 0.005404,   200000],    [-1,-1],[30],[0],[1]],
('WW4.root','_WW4'):      [[1,0, 0.00033931,   200000],    [-1,-1],[30],[0],[1]],
('WW5.root','_WW5'):      [[1,0, 0.0000051484,  38969],    [-1,-1],[30],[0],[1]],
('WZ.root','_WZ'):      [[1,0, 47.13,   1000000],    [-1,-1],[30],[0],[1]],
('ZZ.root','_ZZ'):      [[1,0, 16.523,  642912],    [-1,-1],[30],[0],[1]],
('ST.root','_ST'):      [[1,0, 71.2,    12928026],   [-1,-1],[7],[0],[1]],
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
('fk_data_2016_SingleMuon_SinglePhoton2.root','fk_data'):[[1, 36814,0,0],[-1],[38],[1],[1]],
('fk_ttbar.root','fkttbar'):[[1,0, 831.76, 77044310],  [-1,-1],[50],[0],[-1]],
('fk_DYToLL.root','fkDYToLL'):[[1,0, 5765.4,46380395],  [-1,-1],[5],[0],[-1]],
#('fk_WJets.root','fkWJets'):[[1,0, 61526.7, 97393631],  [-1,-1],[8],[0],[-1]],
('fk_WW1.root','fkWW1'):      [[1,0, 12.178,   1565922],    [-1,-1],[30],[0],[-1]],
...
('fk_ZZ.root','fkZZ'):      [[1,0, 16.523,  642912],    [-1,-1],[30],[0],[-1]],
('fk_ST.root','fkST'):      [[1,0, 71.2,    12928026],   [-1,-1],[7],[0],[-1]],
}

#(file name input,hist name output):[[name list],[nomilazed number],[color],[is data],[+/- factor],[legend],[abs]]
fnamedic_part3={
('ttbar.root','_ttbar'):[['_ttbar'],[0],[84],[0],[1],['t#bar{t}'],[0]],
#('WJets.root','_WJets'):[['_WJets'],[0],[89],[0],[1],['WJets']],
('Null','_TMP'):[['_ST','_WW1','_WW2','_WW3','_WW4','_WW5','_WZ','_ZZ','_DYToLL'],[0],[64],[0],[1],['ST,WW,WZ,ZZ,DY'],[0]],
('Fake_rate','_Fake'):[['fk_data','fkttbar','fkST','fkWW1','fkWW2','fkWW3','fkWW4','fkWW5','fkWZ','fkZZ','fkDYToLL'],[0],[89],[0],[1],['Fake rate'],[1]],
}
