...
def Fill_hist(sample_dic, value_dic, cut):
	...
	for event in t1:
		...
		if not isData:
			...
			MuonSF_weight *= Get_SF_1D(dic_Muon_Tracking_eta,getattr(event,"muon_eta"))
			ElectronSF_weight *= Get_SF_2D(ElectronIDSF_dic,getattr(event,"ele_Et"),getattr(event,"ele_eta"))
			ElectronSF_weight *= Get_SF_2D(dic_Ele_Tracking_pt_eta,getattr(event,"ele_Et"),getattr(event,"ele_eta"))
		...
		for path in value_dic:
			...
			event_weight = MuonSF_weight * ElectronSF_weight * pu_weight * fake_weight * other_weight
		...
		for part in plot_dic:
			for plot in plot_dic[part]:
				...
				if plot_dic[part][plot]["ABS"]: 
					#print path
					my_abs(plot_dic[part][plot]["hist"][path])
				#if plot == "fake_rate":
...
def Get_part_hist_fk(input_dic, value_dic, plot_dic_fk):
	for path in value_dic:
		for part in plot_dic_fk:
			for plot in plot_dic_fk[part]:
				hist_list = plot_dic_fk[part][plot]["data_list"]
				for sample in hist_list:
					plot_dic_fk[part][plot]["hist"][path].Add(plot_dic_fk[part][plot]["hist"][path], input_dic[sample]["hist"][path], 1, 1)
				if plot_dic_fk[part][plot]["ABS"]: 
					#print path
					my_abs(plot_dic_fk[part][plot]["hist"][path])
				#if plot == "fake_rate":
					#my_setErrorzero(plot_dic_fk[part][plot]["hist"][path])

	for part in plot_dic_fk:
		for plot in plot_dic_fk[part]:
			hist_list = plot_dic_fk[part][plot]["data_list"]
			for sample in hist_list:
				plot_dic_fk[part][plot]["N_total"]+=input_dic[sample]["N_norm"]
...
def main_plot_part(path, input_dic, plot_dic, cut, c1):
	...
	legend1=ROOT.TLegend(0.53,0.6,0.73,0.88)
	...
	for plot in plot_dic["compare_3"]:
		plot_dic["compare_3"][plot]["hist"][path].SetFillColor(0)
		plot_dic["compare_3"][plot]["hist"][path].SetLineWidth(3)
		plot_dic["compare_3"][plot]["hist"][path].SetLineStyle(plot_dic["compare_3"][plot]["line_style"])
		plot_dic["compare_3"][plot]["hist"][path].Draw("same:hist")
		legend3.AddEntry(plot_dic["compare_3"][plot]["hist"][path],plot_dic["compare_3"][plot]["legend_title"],"l")
	...
	c1.Print("%s/%s_hratio_%s.pdf"%(cut,cut[1:],path))

def main_plot_part_fk(path, input_dic, plot_dic, cut, Label, c1):
	pad1=ROOT.TPad("pad1", "pad1", 0, 0.315, 1, 0.99 , 0)#used for ratio, the hist plot
	pad2=ROOT.TPad("pad2", "pad2", 0, 0.0, 1, 0.3 , 0)#used for ratio, the ratio plot
	pad1.SetBottomMargin(0.0)
	pad1.SetLeftMargin(0.14)
	...
	#for p in sorted(plot_dic["compare_2"].iteritems(),key=lambda d:d[1]["N_total"],reverse = False):
		#plot = p[0]
	for i in range(len(plot_list_fk)):
		plot = plot_list_fk[len(plot_list_fk)-1-i]
		#print plot_dic["compare_2"][plot]["N_total"]
		compare2_sum.Add(compare2_sum, plot_dic["compare_2"][plot]["hist"][path], 1, 1)
		h_stack.Add(plot_dic["compare_2"][plot]["hist"][path])
	#for p in sorted(plot_dic["compare_2"].iteritems(),key=lambda d:d[1]["N_total"],reverse = True):
		#plot = p[0]
	for plot in plot_list_fk:
		legend1.AddEntry(plot_dic["compare_2"][plot]["hist"][path],plot_dic["compare_2"][plot]["legend_title"],"f")
	...
	tText_5.SetTextFont(40)
	tText_5.SetTextSize(0.03)
	tText_5.AddText ('fake rate %s'%(cut_name[cut][1]))
	tText_5.Draw()
	...
	#c1.cd()
	c1.Update()
	c1.Print("%s/%s_hratio_%s.png"%(Label,Label[1:],path))
	c1.Print("%s/%s_hratio_%s.pdf"%(Label,Label[1:],path))
...
def Init_hist(input_dic, value_dic, plot_dic):
	for path in value_dic:
		if value_dic[path]["use_array"]:
			...
			for part in plot_dic_fke:
				for plot in plot_dic_fke[part]:
					plot_dic_fke[part][plot]["hist"][path] = ROOT.TH1F("%s_%s"%(sample,value_dic[path]["hist_name"]), "", value_dic[path]["hist_para"][0], value_dic[path]["hist_para"][1])
			for part in plot_dic_fkm:
				for plot in plot_dic_fkm[part]:
					plot_dic_fkm[part][plot]["hist"][path] = ROOT.TH1F("%s_%s"%(sample,value_dic[path]["hist_name"]), "", value_dic[path]["hist_para"][0], value_dic[path]["hist_para"][1])
		else:
			for sample in input_dic:
			...
			for part in plot_dic_fke:
				for plot in plot_dic_fke[part]:
					plot_dic_fke[part][plot]["hist"][path] = ROOT.TH1F("%s_%s"%(sample,value_dic[path]["hist_name"]), "", value_dic[path]["hist_para"][0], value_dic[path]["hist_para"][2], value_dic[path]["hist_para"][3])
			for part in plot_dic_fkm:
				for plot in plot_dic_fkm[part]:
					plot_dic_fkm[part][plot]["hist"][path] = ROOT.TH1F("%s_%s"%(sample,value_dic[path]["hist_name"]), "", value_dic[path]["hist_para"][0], value_dic[path]["hist_para"][2], value_dic[path]["hist_para"][3])
...
def Set_hist_fk(input_dic, value_dic, plot_dic_fk):
	for path in value_dic:
		for part in plot_dic_fk:
			for plot in plot_dic_fk[part]:
				plot_dic_fk[part][plot]["hist"][path].Sumw2()
				plot_dic_fk[part][plot]["hist"][path].SetFillColorAlpha(plot_dic[part][plot]["color"],1.0)
				plot_dic_fk[part][plot]["hist"][path].SetLineColorAlpha(plot_dic[part][plot]["color"],1.0)
...
def main_plot(input_dic, value_dic, plot_dic, cut, isPlot = True):
	...
	for path in value_dic:
		...
		main_plot_part(path, input_dic, plot_dic, cut, c1)
	c1.Close()
	...
	Set_hist_fk(input_dic, value_dic, plot_dic_fke)
	Get_part_hist_fk(input_dic, value_dic, plot_dic_fke)
	c1 = ROOT.TCanvas( 'c1', 'A Simple Graph with error bars', 50,50,865,780 )
	c1.SetLeftMargin(0.5)

	for path in value_dic:
		if path == "mass_err_stat":continue
		main_plot_part_fk(path, input_dic, plot_dic_fke, cut, cut_fk, c1)
	c1.Close()
	#~~~~~~~~~~~~~~fake rate muon
	cut_fk = "_FKM"+cut
	try:os.mkdir(cut_fk)
	except:pass
	Set_hist_fk(input_dic, value_dic, plot_dic_fkm)
	Get_part_hist_fk(input_dic, value_dic, plot_dic_fkm)
	c1 = ROOT.TCanvas( 'c1', 'A Simple Graph with error bars', 50,50,865,780 )
	c1.SetLeftMargin(0.5)

	for path in value_dic:
		if path == "mass_err_stat":continue
		main_plot_part_fk(path, input_dic, plot_dic_fkm, cut, cut_fk, c1)
	c1.Close()
...
"compare_2":{
			...
			"Z":{
					"data_list":['_DYToLL_1','_DYToLL_2','_DYToLL_3','_DYToLL_4','_DYToLL_5','_DYToLL_6','_DYToLL_7','_DYToLL_8','_DYToLL_9'],
					"color":ROOT.kBlue-3,
					"legend_title":"DY",
					"N_total":0.0,
					"hist":{},
			...
			"Di_boson":{
					"data_list":['_WW1','_WW2','_WW3','_WW4','_WW5', '_WZ', '_ZZ'],
			...
			"fake_rate_e":{
					"data_list":['fke_data','fke_ttbar_1','fke_ttbar_2','fke_ttbar_3','fke_ttbar_4','fke_ttbar_5','fke_ST','fke_WW1','fke_WW2','fke_WW3','fke_WW4','fke_WW5','fke_WZ','fke_ZZ','fke_DYToLL_1','fke_DYToLL_2','fke_DYToLL_3','fke_DYToLL_4','fke_DYToLL_5','fke_DYToLL_6','fke_DYToLL_7','fke_DYToLL_8','fke_DYToLL_9'],
					"color":ROOT.kYellow,
					"legend_title":"Jet fake e",
					"N_total":0.0,
					"hist":{},
					"ABS":True,
					},
			"fake_rate_m":{
					"data_list":['fkm_data','fkm_ttbar_1','fkm_ttbar_2','fkm_ttbar_3','fkm_ttbar_4','fkm_ttbar_5','fkm_ST','fkm_WW1','fkm_WW2','fkm_WW3','fkm_WW4','fkm_WW5','fkm_WZ','fkm_ZZ','fkm_DYToLL_1','fkm_DYToLL_2','fkm_DYToLL_3','fkm_DYToLL_4','fkm_DYToLL_5','fkm_DYToLL_6','fkm_DYToLL_7','fkm_DYToLL_8','fkm_DYToLL_9'],
					"color":ROOT.kRed,
					"legend_title":"Jet fake #mu",
					"N_total":0.0,
					"hist":{},
					"ABS":True,
					},
			},
"compare_3":{"Z_prime_M1000":{
...
"compare_2":{
			"_ttbar":{
					"data_list":["fke_ttbar_1","fke_ttbar_2","fke_ttbar_3","fke_ttbar_4","fke_ttbar_5"],
					"color":ROOT.kRed-4,
					"legend_title":"t#bar{t}",
					"N_total":0.0,
					"hist":{},
			...
			"Z":{
					"data_list":['fke_DYToLL_1','fke_DYToLL_2','fke_DYToLL_3','fke_DYToLL_4','fke_DYToLL_5','fke_DYToLL_6','fke_DYToLL_7','fke_DYToLL_8','fke_DYToLL_9'],
					"color":ROOT.kBlue,
					"legend_title":"DY",
					"N_total":0.0,
					"hist":{},
			...
			"Di_boson":{
					"data_list":['fke_WW1','fke_WW2','fke_WW3','fke_WW4','fke_WW5', 'fke_WZ', 'fke_ZZ'],
					"color":ROOT.kOrange-3,
					"legend_title":"Diboson",
					"N_total":0.0,
					"hist":{},
...
"compare_2":{
			"_ttbar":{
					"data_list":["fkm_ttbar_1","fkm_ttbar_2","fkm_ttbar_3","fkm_ttbar_4","fkm_ttbar_5"],
					"color":ROOT.kRed-4,
					"legend_title":"t#bar{t}",
					"N_total":0.0,
					"hist":{},
			...
			"Z":{
					"data_list":['fkm_DYToLL_1','fkm_DYToLL_2','fkm_DYToLL_3','fkm_DYToLL_4','fkm_DYToLL_5','fkm_DYToLL_6','fkm_DYToLL_7','fkm_DYToLL_8','fkm_DYToLL_9'],
					"color":ROOT.kBlue,
					"legend_title":"DY",
					"N_total":0.0,
					"hist":{},
			...
			"Di_boson":{
					"data_list":['fkm_WW1','fkm_WW2','fkm_WW3','fkm_WW4','fkm_WW5', 'fkm_WZ', 'fkm_ZZ'],
					"color":ROOT.kOrange-3,
					"legend_title":"Diboson",
					"N_total":0.0,
					"hist":{},
...
plot_list = ["_ttbar","Di_boson","ST","Z","fake_rate_e","fake_rate_m"]
plot_list_fk = ["_ttbar","Di_boson","ST","Z"]
# start run
cut_name={
'_ALL':["e#mu",""],
...
dic_Muon_Tracking_eta           = Load_json("MuonSF/Tracking/Muon_Tracking_json.txt")
dic_Ele_Tracking_pt_eta         = Load_json("MuonSF/Electron/Tracking/Ele_Tracking_json.txt")
ElectronIDSF_dic = {
					(0,10000):{
								(0,1.4442):0.971,
								(1.556,2.4):0.982,
