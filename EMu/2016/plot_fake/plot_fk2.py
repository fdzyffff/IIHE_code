...
def Fill_hist(sample_dic, value_dic, cut):
	...
	for event in t1:
		...
		for path in value_dic:
			for value in value_dic[path]["value_dic"]:
				exec 'passed = (%s)'%(value_dic[path]["value_dic"][value])
				if not passed: continue 
				total_weight = 1.0
				MuonSF_weight = 1.0
...
def	main_plot_part(path, input_dic, cut, c1):
	pad1=ROOT.TPad("pad1", "pad1", 0, 0.1, 1, 1.0 , 0)#used for ratio, the hist plot
	...
	h_stack.Draw("hist")
        h_stack.GetXaxis().SetTitle(value_dic[path]["x_label"][0])
        h_stack.GetXaxis().SetTitleSize(0.05)
        #h_stack.GetXaxis().SetTitleOffset(0.65)
	...
	tText_3=ROOT.TPaveText(0.6,0.9,0.75,1.0,"blNDC")
	...
	c1.Print("%s/hsum_%s.png"%(cut,path))
...
"M_emu_massDep":{"value_dic":{"M_emu":True},
...
"Barrel_Et":{"value_dic":{"t_Et":'getattr(event,"t_region") == 1',"muon_pt":'getattr(event,"muon_region")'},
		"hist_name":"Barrel_Et",
...
"Heep_eta":{"value_dic":{"t_eta":True},
                "hist_name":"HEEP_eta",
                "hist_title":"HEEP eta",
                "use_array":False,
                "PU_reweight":True,
                "hist_para":[51,x_pt,-2.55,2.55],
                "y_axis":["null",300],
                "x_label":['HEEP #eta',0.1],
                "y_label":['Event',0.05],
                "x_log":False,
                "y_log":False,
                "lenend":{
...
"Heep_phi":{"value_dic":{"t_phi":True},
                "hist_name":"HEEP_phi",
                "hist_title":"HEEP phi",
                "use_array":False,
                "PU_reweight":True,
                "hist_para":[35,x_pt,-3.5,3.5],
                "y_axis":["null",360],
                "x_label":['HEEP #phi',0.1],
                "y_label":['Event',0.05],
                "x_log":False,
                "y_log":False,
                "lenend":{
...
"Muon_pt":{"value_dic":{"muon_pt":True},
                "hist_name":"Muon_pt",
                "hist_title":"Muon pT",
                "use_array":True,
                "PU_reweight":True,
...
"Muon_eta":{"value_dic":{"muon_eta":True},
                "hist_name":"Muon_eta",
                "hist_title":"Muon eta",
                "use_array":False,
                "PU_reweight":True,
...
"Muon_phi":{"value_dic":{"muon_phi":True},
                "hist_name":"Muon_phi",
                "hist_title":"Muon phi",
                "use_array":False,
                "PU_reweight":True,
...
"compare_2":{
			...
			"Di_boson":{
					"data_list":['fk_ST','fk_WW1','fk_WW2','fk_WW3','fk_WW4','fk_WW5','fk_WZ','fk_ZZ','fk_DYToLL'],
