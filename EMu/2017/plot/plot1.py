...
def main_plot():
	...
	p_norminal.Store_hist_plot_total()
	...
	if use_sys:
		for tmp_sys_type in sys_dic:
			...
			if (sys_dic[tmp_sys_type][2]):
				...
				sys_class_dic[tmp_sys_type].set_common_hist("qcd_jet", p_norminal.plot_dic["compare_2"]["qcd_jet"])
				if (sys_dic[tmp_sys_type][1]):
...
#	#~~~~~~~~~~~~~~same sign
	if QCD_JET_BKG_TYPE == "same sign":
		...
		p_norminal_ss.set_plot_dic(pre_plot_dic_ss, "12")
		p_norminal_ss.set_input_dic(pre_input_dic, "-1")
		for tmp_sample in p_norminal_ss.input_dic:
			if p_norminal_ss.input_dic[tmp_sample]["weight_factor"] < 0:
	...
	if QCD_JET_BKG_TYPE == "fake rate":
		...
		p_norminal_fke = hist_make()
		p_norminal_fke.sys_type = "norminal_fke"
		p_norminal_fke.set_value_dic(pre_value_dic)
		p_norminal_fke.set_plot_dic(pre_plot_dic_fke, "12")
		p_norminal_fke.set_input_dic(pre_input_dic, "-1")
		for tmp_sample in p_norminal_fke.input_dic:
			p_norminal_fke.input_dic[tmp_sample]["weight_factor"] = 1
		p_norminal_fke.cut_str = cut_dic[cut]
		p_norminal_fke.hist_input_dir = os.path.join(cut,"norminal")
		p_norminal_fke.hist_output_dir = os.path.join(plot_name_fke)
		p_norminal_fke.set_dir()
		p_norminal_fke.total_lumi = p_norminal.Get_total_lumi(p_norminal.input_dic)
	
		p_norminal_fke.main_plot()
		p_norminal_fke.text_list = []
		p_norminal_fke.text_list.append(cut_name[cut][0])
		p_norminal_fke.text_list.append(cut_name[cut][1])
		p_norminal_fke.text_list.append("Fake electron")
	
		for path in p_norminal_fke.value_dic:
			if path == "mass_err_stat":continue
			main_plot_part(p_norminal_fke, path, plot_name_fke)
	#	#~~~~~~~~~~~~~~fake rate muon
		use_sys = False
		plot_name_fkm = "_FKM"+cut
		p_norminal_fkm = hist_make()
		p_norminal_fkm.sys_type = "norminal_fkm"
		p_norminal_fkm.set_value_dic(pre_value_dic)
		p_norminal_fkm.set_plot_dic(pre_plot_dic_fkm, "12")
		p_norminal_fkm.set_input_dic(pre_input_dic, "-1")
		for tmp_sample in p_norminal_fkm.input_dic:
			p_norminal_fkm.input_dic[tmp_sample]["weight_factor"] = 1
		p_norminal_fkm.cut_str = cut_dic[cut]
		p_norminal_fkm.hist_input_dir = os.path.join(cut,"norminal")
		p_norminal_fkm.hist_output_dir = os.path.join(plot_name_fkm)
		p_norminal_fkm.set_dir()
		p_norminal_fkm.total_lumi = p_norminal.Get_total_lumi(p_norminal.input_dic)
	
		p_norminal_fkm.main_plot()
		p_norminal_fkm.text_list = []
		p_norminal_fkm.text_list.append(cut_name[cut][0])
		p_norminal_fkm.text_list.append(cut_name[cut][1])
		p_norminal_fkm.text_list.append("Fake muon")
	
		for path in p_norminal_fkm.value_dic:
			if path == "mass_err_stat":continue
			main_plot_part(p_norminal_fkm, path, plot_name_fkm)
	
	#	#~~~~~~~~~~~~~~fake rate ele + muon
		use_sys = False
		plot_name_fk = "_FK"+cut
		p_norminal_fk = hist_make()
		p_norminal_fk.sys_type = "norminal_fk"
		p_norminal_fk.set_value_dic(pre_value_dic)
		p_norminal_fk.set_plot_dic(pre_plot_dic_fk, "12")
		p_norminal_fk.set_input_dic(pre_input_dic, "-1")
		for tmp_sample in p_norminal_fk.input_dic:
			p_norminal_fk.input_dic[tmp_sample]["weight_factor"] = 1
		p_norminal_fk.cut_str = cut_dic[cut]
		p_norminal_fk.hist_input_dir = os.path.join(cut,"norminal")
		p_norminal_fk.hist_output_dir = os.path.join(plot_name_fk)
		p_norminal_fk.set_dir()
		p_norminal_fk.total_lumi = p_norminal.Get_total_lumi(p_norminal.input_dic)
	
		p_norminal_fk.main_plot()
		p_norminal_fk.text_list = []
		p_norminal_fk.text_list.append(cut_name[cut][0])
		p_norminal_fk.text_list.append(cut_name[cut][1])
		p_norminal_fk.text_list.append("Fake rate")
	
		for path in p_norminal_fk.value_dic:
			if path == "mass_err_stat":continue
			main_plot_part(p_norminal_fk, path, plot_name_fk)
