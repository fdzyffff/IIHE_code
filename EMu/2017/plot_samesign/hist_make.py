...
class hist_make():
	def __init__(self):
		...
		self.plot_list = ["_ttbar","ST","Di_boson","Z","qcd_jet"]#,"fake_rate_e"],"fake_rate_m"]
	...
	def Store_hist_plot(self, input_dic, value_dic, plot_dic, hist_output_dir, path_str = ""):
		for part in plot_dic:
			for plot in plot_dic[part]:
				tmp_plot_dic = plot_dic[part][plot]
				root_file = "%s/plot_hist_%s.root"%(hist_output_dir,plot)
				f1 = ROOT.TFile(root_file,"RECREATE")
				for path in value_dic:
					if (path_str != "" and path_str != path):continue
	...
	def Init_hist_plot(self, value_dic, plot_dic):
		for path in value_dic:
			if value_dic[path]["use_array"]:
				for part in plot_dic:
					for plot in plot_dic[part]:
	...
	def set_input_dic(self, tmp_dic, force_set = False):
		self.input_dic = {}
	...
	def check_sys_sample(self,sample_name):
	...
	def set_plot_dic(self, tmp_dic, force_set = False):
		self.plot_dic = {}
		if self.sys_type == "norminal" or force_set:
			for part in tmp_dic:
			...
			part = "compare_2"
			self.plot_dic[part] = {}
			for plot in tmp_dic[part]:
				if plot in self.plot_list_sys:
					self.plot_dic[part][plot] = deepcopy(tmp_dic[part][plot])
	...
	def main_plot(self):
		...
		if self.sys_type == "norminal": self.Store_hist_plot(self.input_dic, self.value_dic, self.plot_dic, self.hist_output_dir, "M_emu_massDep")
