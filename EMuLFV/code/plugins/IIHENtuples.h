...
#ifndef IIHENtuples_h
#define IIHENtuples_h
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   float w_WW_down ;
   float w_top_PDF_up ;
   float w_top_PDF_down ;
   float w_top_Qscale_up ;
   float w_top_Qscale_down ;
   float w_other ;
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   bool isFake_e;
   bool isFake_mu;
   bool sys_mu_pt_scale_up;
   bool sys_mu_pt_scale_down;
   bool sys_mu_pt_res_up;
   bool sys_ele_et_res_up;
   bool sys_ele_et_scale_up;
   bool sys_ele_et_scale_down;
   float Mass_treshold ;
   ...
   virtual bool     Loop();
   virtual void     displayProgress(long current, long max);
   virtual void     Init();
};

#endif

#ifdef IIHENtuples_cxx
IIHENtuples::IIHENtuples()
{
...
// used to generate this class and read the Tree.
   ...
   triggerVersion = 0 ;
   year = "0000";
   ttbar_reweight = false;
   WW_shape = false;
   isFake_e = false;
   isFake_mu = false;
   sys_mu_pt_scale_up = false;
   sys_mu_pt_scale_down = false;
   sys_mu_pt_res_up = false;
   sys_ele_et_res_up = false;
   sys_ele_et_scale_up = false;
   sys_ele_et_scale_down = false;
   Mass_treshold=9999.0;
}
...
void IIHENtuples::Init()
{
   std::cout<<"year: "<<year<<std::endl;
   std::cout<<"isData: "<<isData<<std::endl;
   std::cout<<"isDY: "<<isDY<<std::endl;
   std::cout<<"isTTbar: "<<isTTbar<<std::endl;
   std::cout<<"isWW: "<<isWW<<std::endl;
   std::cout<<"runNo1: "<<runNo1<<std::endl;
   std::cout<<"runNo2: "<<runNo2<<std::endl;
   std::cout<<"No_print: "<<No_print<<std::endl;
   std::cout<<"mc_str: "<<mc_str<<std::endl;
   std::cout<<"sys_str: "<<sys_str<<std::endl;
   std::cout<<"triggerVersion: "<<triggerVersion<<std::endl;
   std::cout<<"isFake_e: "<<isFake_e<<std::endl;
   std::cout<<"isFake_mu: "<<isFake_mu<<std::endl;
...
Int_t IIHENtuples::GetEntry(Long64_t entry)
{
...
Long64_t IIHENtuples::GetEntries()
{
// Read contents of entry.
   if (!fChain) return 0;
...
void IIHENtuples::Init(TTree *tree)
{
...
//   fChain->SetBranchAddress("ev_rho_kt6PFJetsForIsolation", &ev_rho_kt6PFJetsForIsolation, &b_ev_rho_kt6PFJetsForIsolation);
   if(isData==false && fChain->GetListOfBranches()->FindObject("mc_n")){
...
//   fChain->SetBranchAddress("jet_isJetIDTight", &jet_isJetIDTight, &b_jet_isJetIDTight) ;
   ...
   if (year == "2016") {
      ...
      for(i = 0; i<tmp_array->GetEntries(); i++)
      {
         if(strstr(tmp_array->At(i)->GetName(),"trig_HLT_DoubleEle33_CaloIdL_GsfTrkIdVL_hltDiEle33CaloIdLGsfTrkIdVLDPhiUnseededFilter_eta"))
         {
            cout<<"branch : trig_HLT_DoubleEle33_CaloIdL_GsfTrkIdVL Filters included"<<endl;
         ...
         if(strstr(tmp_array->At(i)->GetName(),"trig_HLT_DoubleEle33_CaloIdL_MW_hltDiEle33CaloIdLMWPMS2UnseededFilter_eta"))
         {
            cout<<"branch : trig_HLT_DoubleEle33_CaloIdL_MW Filters included"<<endl;
      ...
      for(i = 0; i<tmp_array->GetEntries(); i++)
      {
         ...
         //cout<< tmp_array->At(i)->GetName()<<endl;
         if(strstr(tmp_array->At(i)->GetName(),"HLT_OldMu100"))
         {
            cout<<"branch : HLT_OldMu100 included"<<endl;
...
void IIHENtuples::displayProgress(long current, long max){
  using std::cerr;
