...
def Fill_hist(part_sample_dic, hist_dic, cut):
	...
	for root_file in root_file_list:
		...
		for event in t1:
			...
			for path in hist_dic:
				map_fill_hist(event, part_sample_dic["hist"], hist_dic, path)
		f1.Close()
		if Flag_break:break
...
def Get_hist_from_tmp(sample, sample_dic, hist_dic, cut):
	...
	for path in hist_dic:
		sample_dic[sample]["hist"][path]["h_numerator"] = f1.Get("%s_%s_n_out"%(sample,hist_dic[path]["hist_name"]))
		sample_dic[sample]["hist"][path]["h_denominator"] = f1.Get("%s_%s_d_out"%(sample,hist_dic[path]["hist_name"]))
	f1.Close()
	...
	for sample in sample_dic:
		...
		if isSplit_mode:
			if sample == options.sample_name:
				Fill_hist(part_sample_dic, hist_dic, cut)
...
def Store_hist_together(hist_dic, cut):
	try:os.mkdir(cut)
	except:pass
	...
	for path in hist_dic:
		for hist in hist_list:
			if "_out" in hist_dic[path]["hist"][hist].GetName()[-4:]:
				hist_dic[path]["hist"][hist].SetName(hist_dic[path]["hist"][hist].GetName()[:-4])
			tmp_name = hist_dic[path]["hist"][hist].GetName()
			hist_dic[path]["hist"][hist].SetName("%s_out"%(hist_dic[path]["hist"][hist].GetName()))
			hist_dic[path]["hist"][hist].Write()
			hist_dic[path]["hist"][hist].SetName(tmp_name)
	f1.Close()
...
def Store_hist_split(sample_dic, hist_dic, cut):
	try:os.mkdir(cut)
	except:pass
...
def Fill_hist_dic(sample_dic, hist_dic, plot_dic):
	for path in hist_dic:
		for hist in hist_list:
			for sample in sample_dic:
				hist_dic[path]["hist"][hist].Add(hist_dic[path]["hist"][hist], sample_dic[sample]["hist"][path][hist], 1, 1)
...
def main_plot_part(sample_dic, plot_part_dic, hist_dic, cut):
	#start plot
	...
	for p in sorted(plot_part_dic["hist"].iteritems(),key=lambda d:d[1]["legend_order"],reverse = False):
		...
		gr_eff_dic[path].Divide(hist_dic[path]["hist"]["h_numerator"],hist_dic[path]["hist"]["h_denominator"],"cl=0.683 b(1,1) mode")
...
def Set_hist(sample_dic, hist_dic, plot_dic):
	for path in hist_dic:
		for sample in sample_dic:
			sample_dic[sample]["hist"][path]["h_numerator"].Sumw2()
			sample_dic[sample]["hist"][path]["h_denominator"].Sumw2()
		hist_dic[path]["hist"]["h_numerator"].Sumw2()
		hist_dic[path]["hist"]["h_denominator"].Sumw2()
...
def main_plot(sample_dic, hist_dic, plot_dic, cut, isPlot = True):
	...
	Set_hist(sample_dic, hist_dic, plot_dic)
	...
	#get hist for hist_dic
	Fill_hist_dic(sample_dic, hist_dic, plot_dic)
	...
	for plot_part in plot_dic:
		main_plot_part(sample_dic, plot_dic[plot_part], hist_dic, cut)
