...
cut = ""
def get_nEntries_dir(root_file_dir):
    n = 0 
    for root_file_name in os.listdir(root_file_dir):
        root_file = os.path.join(root_file_dir,root_file_name)
...
def get_plot_dic(check_dir = ""):
    ...
    for sample in sample_dic:
        ...
        if isCheck:
            Sample_plot_dic[sample] = [get_nEntries(sample_dic[sample]["input_file"]),[my_walk_dir(os.path.join(check_dir,"split"),[],True)]]
            cut = check_dir.replace("/","")
        else:
            Sample_plot_dic[sample] = [get_nEntries_dir(sample_dic[sample]["input_file_dir"]),[]]
...
def make_sub(label,n_per_job):
    ...
    for sample in Sample_plot_dic:
        ...
        for i in range(1,Sample_plot_dic[sample][0],n_per_job):
            ...
            job_text+=("python plot1.py -s %s --n_range_l %s --n_range_h %s\n"%(sample, n_l, n_h))
            n_job += 1
            n_total_job += 1
            tmp_label = "%s_%s"%(sample,n_job)
            tmp_jobname="Teff_%s.jobb"%(tmp_label)
