...
def Fill_hist(sample_dic, value_dic_pvn, cut):
	...
	for event in t1:
		...
		for path in value_dic_pvn:
			tmp_value_dic_pvn = {}
			if isData:
				tmp_value_dic_pvn = value_dic_pvn[path]["Data_value_dic"]
			else:
				tmp_value_dic_pvn = value_dic_pvn[path]["MC_value_dic"]

			if value_dic_pvn[path]["PU_reweight"]:
			...
			map_value(path, event, sample_dic["hist"][path],tmp_value_dic_pvn,event_weight)

def Get_hist_from_together(sample, sample_dic, value_dic_pvn, cut):
	...
	for path in value_dic_pvn:
		sample_dic["hist"][path] = f1.Get("%s_%s_out"%(sample,value_dic_pvn[path]["hist_name"]))
	f1.Close()

def Get_hist_from_tmp(sample, sample_dic, value_dic_pvn, cut):
...
def Get_hist_from_split(sample, sample_dic, value_dic_pvn, cut):
	root_file = "%s/%s_hist_%s.root"%(cut,cut,sample)
	f1 = ROOT.TFile(root_file,"Read")
	isData = sample_dic["isData"]
	isFake = sample_dic["isFake"]
...
def Get_hist_from_file(sample,sample_dic,value_dic_pvn):
	root_file = sample_dic["input_file"]
	f1 = ROOT.TFile(root_file,"read")
	isData = sample_dic["isData"]
	isFake = sample_dic["isFake"]
...
def Get_hist(input_dic, value_dic_pvn, cut):
	for sample in input_dic:
		sample_dic = input_dic[sample]
		if isSplit_mode:
			if sample == options.sample_name:
				Fill_hist(sample_dic, value_dic_pvn, cut)
		elif (sample_dic["isFromRoot"] and sample_dic["isUpdate"]):
			Fill_hist(sample_dic, value_dic_pvn, cut)
		elif sample_dic["isFromRoot"]:
			Get_hist_from_tmp(sample, sample_dic, value_dic_pvn, cut)
		elif not sample_dic["isFromRoot"]:
			Get_hist_from_file(sample, sample_dic, value_dic_pvn)
		if "pv_n" in value_dic_pvn :
			sample_dic["N_total"] = sample_dic["hist"]["pv_n"].Integral()

def Store_hist_together(input_dic, value_dic_pvn, cut):
	try:os.mkdir(cut)
	except:pass
	root_file = "%s/%s_hist.root"%(cut,cut)
...
def Store_hist_split(input_dic, value_dic_pvn, cut):
	try:os.mkdir(cut)
	except:pass
...
def Norm_hist(input_dic, value_dic_pvn):
...
def Get_part_hist(input_dic, value_dic_pvn, plot_dic):
	for path in value_dic_pvn:
...
def main_plot_part(path, input_dic, cut, c1):
	...
	pad1.SetLogx(value_dic_pvn[path]["x_log"])
	pad1.SetLogy(value_dic_pvn[path]["y_log"])
	pad2.SetLogx(value_dic_pvn[path]["x_log"])
	if value_dic_pvn[path]["use_array"]:
		compare1_sum = ROOT.TH1F("sum_%s"%(value_dic_pvn[path]["hist_name"]), "", value_dic_pvn[path]["hist_para"][0], value_dic_pvn[path]["hist_para"][1])
		compare2_sum = ROOT.TH1F("sum_%s"%(value_dic_pvn[path]["hist_name"]), "", value_dic_pvn[path]["hist_para"][0], value_dic_pvn[path]["hist_para"][1])
	else:
		compare1_sum = ROOT.TH1F("sum_%s"%(value_dic_pvn[path]["hist_name"]), "", value_dic_pvn[path]["hist_para"][0], value_dic_pvn[path]["hist_para"][2], value_dic_pvn[path]["hist_para"][3])
		compare2_sum = ROOT.TH1F("sum_%s"%(value_dic_pvn[path]["hist_name"]), "", value_dic_pvn[path]["hist_para"][0], value_dic_pvn[path]["hist_para"][2], value_dic_pvn[path]["hist_para"][3])
	gr_1 = ROOT.TGraph()

	compare2_sum.SetStats(0)

	h_ratio = ROOT.TH1F()
	h_ratio.SetName("h_ratio_%s"%(value_dic_pvn[path]["hist_name"]))
	...
	for plot in plot_dic["compare_1"]:
		...
		gr_1 = histTograph(compare1_sum, value_dic_pvn[path]["use_array"])
		Graph_Xerror0(gr_1)
		remove_graph_zero(gr_1)
		gr_1.GetXaxis().SetTitle(value_dic_pvn[path]["x_label"][0])
		gr_1.GetXaxis().SetTitleSize(0.1)
		gr_1.GetXaxis().SetTitleOffset(0.95)
		gr_1.GetXaxis().SetLabelSize(value_dic_pvn[path]["x_label"][1])
		gr_1.GetYaxis().SetTitle(value_dic_pvn[path]["y_label"][0])
	...
	if value_dic_pvn[path]["y_axis"][0]!= 'null':h_stack.SetMinimum(value_dic_pvn[path]["y_axis"][0])
	if value_dic_pvn[path]["y_axis"][1]!= 'null':h_stack.SetMaximum(value_dic_pvn[path]["y_axis"][1])
        else:h_stack.SetMaximum(getYmax(compare2_sum))
	h_stack.Draw("hist")
	h_stack.GetYaxis().SetTitle(value_dic_pvn[path]["y_label"][0])
	...
	h_ratio.GetXaxis().SetTitle(value_dic_pvn[path]["x_label"][0])
	...
	gr_ratio = get_graph_ratio(histTograph(compare1_sum, value_dic_pvn[path]["use_array"]), ROOT.TGraphAsymmErrors(compare2_sum))
	...
	#c1.cd()
	c1.Update()
	c1.Print("%s/%s_hratio_%s.png"%(cut,cut,path))
...
def Init_hist(input_dic, value_dic_pvn, plot_dic):
	for path in value_dic_pvn:
		if value_dic_pvn[path]["use_array"]:
			for sample in input_dic:
			...
			for part in plot_dic:
				for plot in plot_dic[part]:
					plot_dic[part][plot]["hist"][path] = ROOT.TH1F("%s_%s"%(sample,value_dic_pvn[path]["hist_name"]), "", value_dic_pvn[path]["hist_para"][0], value_dic_pvn[path]["hist_para"][1])
		else:
			for sample in input_dic:
			...
			for part in plot_dic:
				for plot in plot_dic[part]:
					plot_dic[part][plot]["hist"][path] = ROOT.TH1F("%s_%s"%(sample,value_dic_pvn[path]["hist_name"]), "", value_dic_pvn[path]["hist_para"][0], value_dic_pvn[path]["hist_para"][2], value_dic_pvn[path]["hist_para"][3])

def Set_hist(input_dic, value_dic_pvn, plot_dic):
	for path in value_dic_pvn:
		for sample in input_dic:
...
def print_error(error_range_dic, input_dic, value_dic_pvn, plot_dic):
	error_range_dic = {"0-500":[1,500,0,0], "500-1000":[2,500,0,0], "1000-1500":[3,500,0,0], "1500-2500":[4,1000,0,0]}
	path = "mass_err_stat"
	if value_dic_pvn[path]["use_array"]:
		h_err = ROOT.TH1F("h_err_%s"%(value_dic_pvn[path]["hist_name"]), "", value_dic_pvn[path]["hist_para"][0], value_dic_pvn[path]["hist_para"][1])
	else:
		h_err = ROOT.TH1F("h_err_%s"%(value_dic_pvn[path]["hist_name"]), "", value_dic_pvn[path]["hist_para"][0], value_dic_pvn[path]["hist_para"][2], value_dic_pvn[path]["hist_para"][3])
	for p in sorted(plot_dic["compare_2"].iteritems(),key=lambda d:d[1]["N_total"],reverse = False):
		plot = p[0]
...
def main_plot(input_dic, value_dic_pvn, plot_dic, cut, isPlot = True):
	#Initialize hist
	Init_hist(input_dic, value_dic_pvn, plot_dic)
	#set up hist parameters
	Set_hist(input_dic, value_dic_pvn, plot_dic)
	#get hist from root file using Get() or Fill()
	Get_hist(input_dic, value_dic_pvn, cut)
	#Store hist to root file
	Store_hist(input_dic, value_dic_pvn, cut)
	#Normalize hist
	Norm_hist(input_dic, value_dic_pvn)
	#print contribution of each dataset
	Print_table(input_dic)
	#get hist for plot_dic
	Get_part_hist(input_dic, value_dic_pvn, plot_dic)
	
	#print_error(error_range_dic, input_dic, value_dic_pvn, plot_dic)
	#print_sum(input_dic)
...
def main_plot_split(input_dic, value_dic_pvn, plot_dic, cut, isPlot = True):
	#Initialize hist
	Init_hist(input_dic, value_dic_pvn, plot_dic)
	#set up hist parameters
	Set_hist(input_dic, value_dic_pvn, plot_dic)
	...
	Get_hist(input_dic, value_dic_pvn, cut)
	#Store hist to root file
	Store_hist_split(input_dic, value_dic_pvn, cut)
...
'SF_EE_80':["ee",""],
'SF_MuMu_80':["#mu#mu",""],
'SF_EMu_80_step2':["e#mu","event selection step 2"],
'SF_EE_80_step2':["ee","event selection step 2"],
'SF_MuMu_80_step2':["#mu#mu","event selection step 2"],
...
cut_dic={
#'SF_EMu_80':'getattr(event,"isEMu") and getattr(event,"pass_trigger_EMu") and getattr(event,"pass_step1")',
'SF_EE_80':'getattr(event,"isEE") and getattr(event,"pass_trigger_EE") and getattr(event,"pass_step1")',
#'SF_EMu_80_step2':'getattr(event,"isEMu") and getattr(event,"pass_trigger_EMu") and getattr(event,"pass_step1") and (getattr(event,"M_ll") >= 80 or getattr(event, "MET_T1Txy_significance") >= 4)',
#'SF_EE_80_step2':'getattr(event,"isEE") and getattr(event,"pass_trigger_EE") and getattr(event,"pass_step1") and (getattr(event,"M_ll") < 81 or getattr(event,"M_ll") > 101) and getattr(event,"MET_T1Txy_significance")>=4',
#'SF_MuMu_80_step2':'getattr(event,"isMuMu") and getattr(event,"pass_trigger_MuMu") and getattr(event,"pass_step1") and (getattr(event,"M_ll") < 81 or getattr(event,"M_ll") > 101) and getattr(event,"MET_T1Txy_significance") >=4',

#'SF_EMu_74':'getattr(event,"isEMu") and getattr(event,"pass_trigger_EMu") and getattr(event,"pass_step1")',
#'SF_EE_74':'getattr(event,"isEE") and getattr(event,"pass_trigger_EE") and getattr(event,"pass_step1")',
#'SF_MuMu_74':'getattr(event,"isMuMu") and getattr(event,"pass_trigger_MuMu") and getattr(event,"pass_step1")',
#'SF_EE_74_zrm':'getattr(event,"isEE") and getattr(event,"pass_trigger_EE") and getattr(event,"pass_step1") and (getattr(event,"M_ll") < 81 or getattr(event,"M_ll") > 101',
...
for cut in cut_dic:
	if options.sample_name == "null":
		isSplit_mode = False
		main_plot(input_dic, value_dic_pvn, plot_dic, cut)
	else:
		isSplit_mode = True
		...
		if options.n_range_l<0 or options.n_range_h < 0:
			...
			main_plot_split(input_dic, value_dic_pvn, plot_dic, cut)
		else:
			main_plot_split(input_dic, value_dic_pvn, plot_dic, cut)
