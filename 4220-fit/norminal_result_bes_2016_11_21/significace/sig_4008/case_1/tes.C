...
//start minuit
 ...
 // Set starting values and step sizes for parameters
   ...
   Double_t para[28] = {4.223 , 0.057, 0.2, 4.393, 0.154, 0.6, 3.2, 0.0, 0.0, 0.5, 3.86, 0.31, 0.0 , 4.33 , 0.056 , 0.067 , 0.26, 0.2, 1.86, 1200.0, 0.5, 3.7, 2.0, 1.7, 0, 0, 0, 0};//Default result
   ...
   gMinuit->mnparm( 0, "M(4220)"          , para[0], para_step[0], 4.15, 4.5, ierflg);//4220
   gMinuit->mnparm( 1, "width(4220)"      , para[1], para_step[1], 0, 0, ierflg);//4220
   gMinuit->mnparm( 2, "gaB_hc(4220)"     , para[2], para_step[2], 0, 0, ierflg);//4220
   gMinuit->mnparm( 3, "M(4390)"          , para[3], para_step[3], 0, 0, ierflg);//4390 in pipihc 
   gMinuit->mnparm( 4, "width(4390)"      , para[4], para_step[4], 0, 0, ierflg);//4390 in pipihc 
   gMinuit->mnparm( 5, "gaB(4390)"        , para[5], para_step[5], 0, 0, ierflg);//4390 in pipihc 
   ...
   gMinuit->mnparm( 8, "phase_omega"      , para[8], para_step[8], 0, 0, ierflg);//omega chi_c0
   gMinuit->mnparm( 9, "gaB_omega(4220)"  , para[9], para_step[9], 0, 0, ierflg);//omega chi_c0

   gMinuit->mnparm( 10, "M(4008)"         , para[10], para_step[10], 0, 0, ierflg);
   gMinuit->mnparm( 11, "width(4008)"     , para[11], para_step[11], 0, 0, ierflg);
   gMinuit->mnparm( 12, "gaB(4008)"       , para[12], para_step[12], 0, 0, ierflg);//
   gMinuit->mnparm( 13, "M(4320)"         , para[13], para_step[13], 4.15, 4.5, ierflg);//
   gMinuit->mnparm( 14, "width(4320)"     , para[14], para_step[14], 0, 0, ierflg);//
   gMinuit->mnparm( 15, "gaB(4320)"       , para[15], para_step[15], 0, 0, ierflg);//
   gMinuit->mnparm( 16, "gaB_jpsi(4220)"  , para[16], para_step[16], 0, 0, ierflg);//
