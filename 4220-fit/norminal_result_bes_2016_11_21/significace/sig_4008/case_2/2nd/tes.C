...
Double_t my_cs_jpsi(Double_t mass, Double_t *par_list, bool my_write)
{
   ...
   Double_t phi         = par_list[8];
   Double_t gaB3        = par_list[9];
   Double_t Aa          = par_list[10];
   Double_t Bb      = par_list[11];
   Double_t Cc        = par_list[12];
   Double_t M6          = par_list[13];
   Double_t width6      = par_list[14];
   Double_t gaB6        = par_list[15];
   Double_t gaB4        = par_list[16];
...
//start minuit
 ...
 // Set starting values and step sizes for parameters
   ...
   Double_t para[28] = {4.223 , 0.057, 0.2, 4.393, 0.154, 0.6, 3.2, 0.0, 0.0, 0.5, -0.1, 0.0, 0.0 , 4.33 , 0.056 , 0.067 , 0.26, 0.2, 1.86, 1200.0, 0.5, 3.7, 2.0, 1.7, 0, 0, 0, 0};//Default result
   ...
   gMinuit->mnparm( 3, "M(4390)"          , para[3], para_step[3], 4.15, 4.6, ierflg);//4390 in pipihc 
   ...
   gMinuit->mnparm( 9, "gaB_omega(4220)"  , para[9], para_step[9], 0, 50, ierflg);//omega chi_c0

   gMinuit->mnparm( 10, "A"         	  , para[10], para_step[10], -0.5, 0.5, ierflg);
   gMinuit->mnparm( 11, "B"		  , para[11], para_step[11], 0, 0, ierflg);
   gMinuit->mnparm( 12, "C"        	  , para[12], para_step[12], 0, 0, ierflg);//
   gMinuit->mnparm( 13, "M(4320)"         , para[13], para_step[13], 3.85, 4.5, ierflg);//
   gMinuit->mnparm( 14, "width(4320)"     , para[14], para_step[14], 0, 0, ierflg);//
   gMinuit->mnparm( 15, "gaB(4320)"       , para[15], para_step[15], 0, 50, ierflg);//
   gMinuit->mnparm( 16, "gaB_jpsi(4220)"  , para[16], para_step[16], 0, 50, ierflg);//
