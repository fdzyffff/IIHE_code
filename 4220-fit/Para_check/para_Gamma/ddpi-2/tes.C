...
Double_t my_cs_ddpi(Double_t mass, Double_t *par_list, bool my_write)
{
   ...
   my_BW(mass,M6,width6,gaB10,ps3_mass,ps3_M6, re10, im10);// 5: Y(4390)
...
//start minuit
 ...
 // Set starting values and step sizes for parameters
   ...
   Double_t para[28] = {4.219 , 0.057, 0.4, 4.391, 0.154, 1.2, 3.2, 0.0, 0.0, 0.5, 3.845, 0.35, 0.66 , 4.33 , 0.056 , 0.067 , 0.26, 0.2, 1.86, 1500.0, 4.3, 6.2, -2.2, 1.5, 0, 0, 0, 0};//new data ddpi result 2
   ...
   gMinuit->mnparm( 1, "width(4220)"      , para[1], para_step[1], 0.03, 0.1, ierflg);//4220
   gMinuit->mnparm( 2, "gaB_hc(4220)"     , para[2], para_step[2], 0.01, 5, ierflg);//4220
   gMinuit->mnparm( 3, "M(4390)"          , para[3], para_step[3], 4.39, 4.42, ierflg);//4390 in pipihc 
   gMinuit->mnparm( 4, "width(4390)"      , para[4], para_step[4], 0.01, 0.2, ierflg);//4390 in pipihc 
   gMinuit->mnparm( 5, "gaB(4390)"        , para[5], para_step[5], 0.01, 5, ierflg);//4390 in pipihc 
   ...
   gMinuit->mnparm( 9, "gaB_omega(4220)"  , para[9], para_step[9], 0.01, 10, ierflg);//omega chi_c0
   ...
   gMinuit->mnparm( 12, "gaB(4008)"       , para[12], para_step[12], 0.01, 5, ierflg);//
   gMinuit->mnparm( 13, "M(4320)"         , para[13], para_step[13], 4.3, 4.4, ierflg);//
   gMinuit->mnparm( 14, "width(4320)"     , para[14], para_step[14], 0.03, 1.5, ierflg);//
   gMinuit->mnparm( 15, "gaB(4320)"       , para[15], para_step[15], 0.01, 5, ierflg);//
   gMinuit->mnparm( 16, "gaB_jpsi(4220)"  , para[16], para_step[16], 0.01, 5, ierflg);//
   ...
   gMinuit->mnparm( 20, "gaB_ddpi(4220)"        , para[20], para_step[20], 0.1, 10, ierflg);//
   gMinuit->mnparm( 21, "gaB_ddpi(4390)"        , para[21], para_step[21], 0.1, 10, ierflg);//
   gMinuit->mnparm( 22, "phase_ddpi_(4220)"     , para[22], para_step[22], -4, 4, ierflg);//
   gMinuit->mnparm( 23, "phase_ddpi_(4390)"     , para[23], para_step[23], -4, 4, ierflg);//
