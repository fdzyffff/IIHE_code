...
static Double_t crossSection_jpsi_part4;
static Double_t crossSection_jpsi_part5;
...
Double_t my_cs_hc(Double_t mass, Double_t *par_list, bool my_write)
{
   ...
   Double_t phase_5     = par_list[13];
   Double_t M6          = par_list[14];
   Double_t width6      = par_list[15];
   Double_t gaB6        = par_list[16];
   Double_t gaB4        = par_list[17];
   Double_t phase_4     = par_list[18];
   Double_t phase_6     = par_list[19];
   Double_t CC_ddpi     = par_list[20];
   Double_t gaB7        = par_list[21];
   Double_t gaB8        = par_list[22];
   Double_t phase_7     = par_list[23];
   Double_t phase_8     = par_list[24];
...
Double_t my_cs_jpsi(Double_t mass, Double_t *par_list, bool my_write)
{
   ...
   Double_t M10    	 = par_list[25];
   Double_t width10 	 = par_list[26];
   Double_t gaB10   	 = par_list[27];
   Double_t phase_10     = par_list[28];

   Double_t M11    	 = par_list[29];
   Double_t width11 	 = par_list[30];
   Double_t gaB11   	 = par_list[31];
   ...
   Double_t ps3_M5 = ps3(M5,m1,m2,m3);//4040
   Double_t ps3_M10 = ps3(M10,m1,m2,m3);//4160
   Double_t ps3_M11 = ps3(M11,m1,m2,m3);//3770

   //printf("M10: %f, M11: %f\n",M10,M11);
   //printf("ps3_M10: %f, ps3_M11: %f\n",ps3_M10,ps3_M11);


   my_BW(mass,M1,width1,gaB4,ps3_mass,ps3_M1, re4, im4);	// 4: Y(4220)
   my_BW(mass,M6,width6,gaB6,ps3_mass,ps3_M6, re6, im6);	// 6: Y(4320)
   my_BW(mass,M5,width5,gaB5,ps3_mass,ps3_M5, re5, im5);	// 5: Y(4040)
   my_BW(mass,M10,width10,gaB10,ps3_mass,ps3_M10, re10, im10);	// 10: Y(4160)
   my_BW(mass,M11,width11,gaB11,ps3_mass,ps3_M11, re11, im11);	// 11: Y(3770)

   re=re11+re4*cos(phase_4)-im4*sin(phase_4)+re5*cos(phase_5)-im5*sin(phase_5)+re6*cos(phase_6)-im6*sin(phase_6)+re10*cos(phase_10)-im10*sin(phase_10);
   im=im11+im4*cos(phase_4)+re4*sin(phase_4)+im5*cos(phase_5)+re5*sin(phase_5)+im6*cos(phase_6)+re6*sin(phase_6)+im10*cos(phase_10)+re10*sin(phase_10);
   ...
   if(my_write)
   {
      ...
      crossSection_jpsi_part4 = (pow(re10,2)+pow(im10,2))*(0.389379*10);
      crossSection_jpsi_part5 = (pow(re11,2)+pow(im11,2))*(0.389379*10);
   }
   return crossSection;
}
...
Double_t my_cs_ddpi(Double_t mass, Double_t *par_list, bool my_write)
{
   ...
   re=tt+re7*cos(phase_7)-im7*sin(phase_7)+re8*cos(phase_8)-im8*sin(phase_8);
   im=   im7*cos(phase_7)+re7*sin(phase_7)+im8*cos(phase_8)+re8*sin(phase_8);
...
void my_plot_jpsi(Double_t *par_list)
{
   ...
   TGraph *gr_jpsi_fit_part4 = new TGraph();
   TGraph *gr_jpsi_fit_part5 = new TGraph();
   Int_t i=1;
   Double_t mass;
   for(mass = 3.6; mass < 4.8; mass+= 0.001)
   {
      ...
      gr_jpsi_fit_part4->SetPoint(i,mass,crossSection_jpsi_part4);
      gr_jpsi_fit_part5->SetPoint(i,mass,crossSection_jpsi_part5);
      i+=1;
      //out_jpsi << "%f\t%f\n";
   }
   ...
   gr_jpsi_fit_part1->SetLineWidth(2);
   ...
   gr_jpsi_fit_part2->SetLineWidth(2);
   ...
   gr_jpsi_fit_part3->SetLineWidth(2);
   gr_jpsi_fit_part3->Draw("C");
   gr_jpsi_fit_part4->SetMarkerColor(2);
   gr_jpsi_fit_part4->SetLineColor(kBlue);
   gr_jpsi_fit_part4->SetLineStyle(2);
   gr_jpsi_fit_part4->SetLineWidth(2);
   gr_jpsi_fit_part4->Draw("C");
   gr_jpsi_fit_part5->SetMarkerColor(2);
   gr_jpsi_fit_part5->SetLineColor(kBlue);
   gr_jpsi_fit_part5->SetLineStyle(2);
   gr_jpsi_fit_part5->SetLineWidth(2);
   gr_jpsi_fit_part5->Draw("C");
...
//start minuit
   TMinuit *gMinuit = new TMinuit(32);
   gMinuit->SetFCN( fcn );
 ...
 // Set starting values and step sizes for parameters
   ...
   Double_t para[32] = {4.223 , 0.057, 0.2, 4.393, 0.154, 0.6, 3.2, 0.0, 0.0, 0.5, 4.039, 0.08, 0.085 , 1.21, 4.31 , 0.03  , 0.04  , 6.56, 0.341, 1.13 , 1200.0, 0.5, 3.7, 2.0, 1.7, 4.191, 0.07, 4.67 , -3.14, 3.773, 0.0272 , 0.27 };//Default result
   //Double_t para[28] = {4.223 , 0.057, 0.2, 4.393, 0.154, 0.6, 3.2, 0.0, 0.0, 0.5, 3.86, 0.31, 0.55 , 4.33 , 0.056 , 0.067 , 0.26, 0.2, 1.86, 2000.0, 7.2, 5.9, -2.2, 2.4, 0, 0, 0, 0};//new data ddpi result 2
   //Double_t para[28] = {4.223 , 0.057, 0.2, 4.393, 0.154, 0.6, 3.2, 0.0, 0.0, 0.5, 3.86, 0.31, 0.55 , 4.33 , 0.056 , 0.067 , 0.26, 0.2, 1.86, 2000.0, 2.9, 26.0, -2.2, 4.2, 0, 0, 0, 0};//new data ddpi result 3
   //Double_t para[28] = {4.223 , 0.057, 0.2, 4.393, 0.154, 0.6, 3.2, 0.0, 0.0, 0.5, 3.86, 0.31, 0.55 , 4.33 , 0.056 , 0.067 , 0.26, 0.2, 1.86, 2000.0, 8.4, 35.0, -1.4, 3.94, 0, 0, 0, 0};//new data ddpi result 4

   //Double_t para[28] = {4.223 , 0.057, 0.2, 4.393, 0.154, 0.6, 3.2, 0.0, 0.0, 0.5, 3.86, 0.31, 0.55 , 4.33 , 0.056 , 1.3 , 0.3, 0.9, -1.9, 2000.0, 2.5, 4.36, 3.2, 2.7, 0, 0, 0, 0};//new data jpsi result 2
   //Double_t para[28] = {4.223 , 0.057, 0.2, 4.393, 0.154, 0.6, 3.2, 0.0, 0.0, 0.5, 3.86, 0.31, 0.55 , 4.33 , 0.12 , 0.1 , 0.9 , -2.0 ,1.0, 2000.0, 2.5, 4.36, 3.2, 2.7, 0, 0, 0, 0};//new data jpsi result 3
   //Double_t para[28] = {4.223 , 0.057, 0.2, 4.393, 0.154, 0.6, 3.2, 0.0, 0.0, 0.5, 3.86, 0.31, 0.55 , 4.33 , 0.11 , 1.0 , 1.1, -0.7 ,-2.5, 2000.0, 2.5, 4.36, 3.2, 2.7, 0, 0, 0, 0};//new data jpsi result 4

   //Double_t para[28] = {4.223 , 0.057, 0.2, 4.393, 0.154, 0.6, 3.2, 0.0, 0.0, 0.5, 3.86, 0.31, 0.55 , 4.33 , 0.056 , 0.067 , 0.26, 0.2, 1.86, 2000.0, 7.2, 5.9, -2.2, 2.4, 0, 0, 0, 0};//new data pipihc result 2

   Double_t para_step[32] = {0.001, 0.001, 0.001, 0.001, 0.001, 0.001, 0.001, 0.1, 0.01, 0.01, 0.001, 0.001, 0.01, 0.01, 0.001, 0.001, 0.01, 0.01, 0.01, 0.01, 1, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01};
   ...
   gMinuit->mnparm( 2, "gaB_hc(4220)"     , para[2], para_step[2], 0, 5, ierflg);//4220
   ...
   gMinuit->mnparm( 5, "gaB(4390)"        , para[5], para_step[5], 0, 5, ierflg);//4390 in pipihc 
   ...
   gMinuit->mnparm( 10, "M(4040)"         , para[10], para_step[10], 3.6, 4.2, ierflg);
   gMinuit->mnparm( 11, "width(4040)"     , para[11], para_step[11], 0.0, 0.7, ierflg);
   gMinuit->mnparm( 12, "gaB(4040)"       , para[12], para_step[12], 0, 15, ierflg);//
   gMinuit->mnparm( 13, "phase_jpsi(4040)", para[13], para_step[13], 0, 0, ierflg);//
   gMinuit->mnparm( 14, "M(4320)"         , para[14], para_step[14], 4.2, 4.4, ierflg);//
   gMinuit->mnparm( 15, "width(4320)"     , para[15], para_step[15], 0.03, 0.25, ierflg);//
   gMinuit->mnparm( 16, "gaB(4320)"       , para[16], para_step[16], 0, 15, ierflg);//
   gMinuit->mnparm( 17, "gaB_jpsi(4220)"  , para[17], para_step[17], 0, 50, ierflg);//
   gMinuit->mnparm( 18, "phase_jpsi_(4220)"    , para[18], para_step[18], -5, 5, ierflg);//
   gMinuit->mnparm( 19, "phase_jpsi_(4320)"    , para[19], para_step[19], -5, 5, ierflg);//

   gMinuit->mnparm( 20, "CC_ddpi"               , para[20], para_step[20], 1000, 3000, ierflg);//constant for PS3 in DDpi
   gMinuit->mnparm( 21, "gaB_ddpi(4220)"        , para[21], para_step[21], 0, 50, ierflg);//
   gMinuit->mnparm( 22, "gaB_ddpi(4390)"        , para[22], para_step[22], 0, 50, ierflg);//
   gMinuit->mnparm( 23, "phase_ddpi_(4220)"     , para[23], para_step[23], 0, 0, ierflg);//
   gMinuit->mnparm( 24, "phase_ddpi_(4390)"     , para[24], para_step[24], 0, 0, ierflg);//

   gMinuit->mnparm( 25, "M(4160)"     		, para[25], para_step[25], 0, 0, ierflg);//
   gMinuit->mnparm( 26, "width(4160)"  		, para[26], para_step[26], 0, 50, ierflg);//
   gMinuit->mnparm( 27, "gaB_jpsi(4160)"        , para[27], para_step[27], 0, 50, ierflg);//
   gMinuit->mnparm( 28, "phase_jpsi(4160)"      , para[28], para_step[28], 0, 0, ierflg);//

   gMinuit->mnparm( 29, "M(3770)"     		, para[29], para_step[29], 0, 0, ierflg);//
   gMinuit->mnparm( 30, "width(3770)"  		, para[30], para_step[30], 0, 50, ierflg);//
   gMinuit->mnparm( 31, "gaB_jpsi(3770)"        , para[31], para_step[31], 0, 50, ierflg);//
   ...
   Double_t out_par_list[33] = {0};
   Double_t out_par_err_list[33] = {0};
   ...
   gMinuit->GetParameter(28,out_par_list[28],out_par_err_list[28]);
   gMinuit->GetParameter(29,out_par_list[29],out_par_err_list[29]);
   gMinuit->GetParameter(30,out_par_list[30],out_par_err_list[30]);
   gMinuit->GetParameter(31,out_par_list[31],out_par_err_list[31]);
