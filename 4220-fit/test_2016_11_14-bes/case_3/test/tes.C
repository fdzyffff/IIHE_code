...
Double_t tes_2(Double_t* my_para)
{
   ifstream in1;
   ifstream in2;
   ifstream in3;
...
//start minuit
 ...
 // Set starting values and step sizes for parameters
   ...
   //Double_t para[28] = {4.2205, 0.055, 0.2625, 4.3788, 0.147, 0.882, 2.23, 0.0, 0.0, 0.3379, 3.833, 0.359, 0.547, 4.339, 0.116, 0.68, 0.24, 0.144, 2.35, 1436.0, 0.7, 11.6, 3.1, 1.2, 0.0, 0.0, 20.4, -2.3};//Default result 5
   Double_t para[28] = {4.221, 0.054, 0.49, 4.383, 0.146, 1.386, 3.431, 0.0, 0.0, 0.334, 3.832, 0.358, 0.541, 4.336, 0.113, 0.072, 0.231, 0.158, 2.284, 1349.2, 4.6, 58.0, 5.6, 4.24, 0.0, 0.0, 0.63, 0.40};//Default result 6

   Double_t para_step[28] = {0.001, 0.001, 0.01, 0.001, 0.001, 0.01, 0.01, 0.1, 0.01, 0.01, 0.001, 0.001, 0.01, 0.001, 0.001, 0.01, 0.01, 0.01, 0.01, 1, 0.001, 0.001, 0.001, 0.001, 0.01, 0.01, 0.001, 0.001};
   ...
   gMinuit->mnparm( 19, "CC_ddpi"               , my_para[0], para_step[19], 0, 0, ierflg);//constant for PS3 in DDpi
   gMinuit->mnparm( 20, "gaB_ddpi(4220)"        , my_para[1], para_step[20], 0, 50, ierflg);//
   gMinuit->mnparm( 21, "gaB_ddpi(4390)"        , my_para[2], para_step[21], 0, 150, ierflg);//
   gMinuit->mnparm( 22, "phase_ddpi_(4220)"     , my_para[3], para_step[22], 0, 0, ierflg);//
   gMinuit->mnparm( 23, "phase_ddpi_(4390)"     , my_para[4], para_step[23], 0, 0, ierflg);//

   gMinuit->mnparm( 24, "gaB_hc(4320)"       , para[24], para_step[24], 0, 60, ierflg);//
   gMinuit->mnparm( 25, "phase_hc(4320)"     , para[25], para_step[25], 0, 0, ierflg);//

   gMinuit->mnparm( 26, "gaB_ddpi(4320)"       , my_para[5], para_step[26], 0.0, 150, ierflg);//
   gMinuit->mnparm( 27, "phase_ddpi(4320)"     , my_para[6], para_step[27], 0, 0, ierflg);//
...
bool my_check(Double_t* out_para)
{
   Double_t result_1[5][7] = {{1349,0.5,5.95,2.33,1.68.0.46,-2.71},{1349,4.63,8.96,-2.19,1.28,0.88,3.0},{1349,6.1,43.3,-0.66,-2.28,2.5,-1.0},{1349.2,0.66,29.0,-2.42,-1.87,1.29,-0.44},{1348.75,0.68,40.23,3.07,0.99,50.5,-2.25}}
   bool isNew = true;
   ...
   {
      check_pass = false;
      if(fabs(result_1[j][0]-out_para[0]) > 50)check_pass = true;
      if(fabs(result_1[j][1]-out_para[1]) > 0.5)check_pass = true;
      if(fabs(result_1[j][2]-out_para[2]) > 0.5)check_pass = true;
      //if(fabs(result_1[j][3]-out_para[3]) > 0.3)check_pass = true;
      //if(fabs(result_1[j][4]-out_para[4]) > 0.3)check_pass = true;
      if(fabs(result_1[j][5]-out_para[5]) > 0.5)check_pass = true;
      //if(fabs(result_1[j][6]-out_para[6]) > 0.3)check_pass = true;
      if(!check_pass)isNew == false;
   }
   return isNew
}
int tes()
{
   Double_t my_para[7] = {1349.2, 4.6, 58.0, 5.6, 4.24, 0.63, 0.40};//Default result 6
   for(my_para[0] = 500, my_para[0] < 1500, my_para[0]+=150.)
   {
      for(my_para[1] = 0.5, my_para[1] < 15, my_para[1]+=2.5)
      {
         for(my_para[2] = 1.5, my_para[2] < 60, my_para[2]+=5.)
         {
            for(my_para[3] = -3.3, my_para[3] < 3.3, my_para[3]+=0.3)
            {
              for(my_para[4] = -3.3, my_para[4] < 3.3, my_para[4]+=0.3)
              {
                 for(my_para[5] = 0.1, my_para[5] < 10.0, my_para[0]+=0.5)
                 {
                    for(my_para[6] = -3.3, my_para[6] < 3.3, my_para[0]+=0.3)
                    {
                       if(my_check(tes_2(my_para)))
                       {
                           return 0;
                       }
                    }
