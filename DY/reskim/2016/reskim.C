...
//    accept_EcalDriven_ID = accept_EcalDriven ;
    ...
    //if ((region == 1 && p4.Pt() >= 43) || (region == 3 && p4.Pt() >= 48))
    //{
    //    pass_l1_cut = true;
    //}
    //if(isTag)
...
//      }
      ...
      if(true==isData){
        ...
        w_PU_out       = w_sign*PU_2016::MC_pileup_weight(PU, 0, "all");
      }
...
/*        if(tmp_n_muon==2)
      ...
      for(unsigned int iEl=0 ; iEl<gsf_eta->size() ; ++iEl){
        ...
        if(isData){
          Int_t tmp_trig_dou33_fire;
          vector<float> *tmp_trig_dou33_eta;
          vector<float> *tmp_trig_dou33_phi;
          if (ev_run < 276453 || ev_run > 278822) {
            tmp_trig_dou33_eta = trig_dou33MW_eta;
            tmp_trig_dou33_phi = trig_dou33MW_phi;
            tmp_trig_dou33_fire = trig_DouEle33MW_fire;
          } else {
            tmp_trig_dou33_eta = trig_dou33_eta;
            tmp_trig_dou33_phi = trig_dou33_phi;
            tmp_trig_dou33_fire = trig_DouEle33_fire;  
          }
          ...
          el->pass_trigger = (tmp_trig_dou33_fire && tmp_trigger_pass);
          el->match_trigger_l1_dou33 = true ;
        }
        else{
          el->pass_trigger = trigEle33_2016::passTrig(el->Et, eta) ;
          el->match_trigger_l1_dou33 = true;
        }
