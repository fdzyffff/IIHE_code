...
def get_1D_json(file_name, hist_name, xy_convert = False, isPrint = True):
    SF_dic = {}
    f1 = ROOT.TFile.Open(file_name,"READ")
    h1 = f1.Get(hist_name)
    ...
    for i in range(1,x_axis.GetNbins()+1):
        ...
        x_u = x_axis.GetBinUpEdge(i)
        x_l = x_axis.GetBinLowEdge(i)
        SF_dic[(x_l,x_u)] = h1.GetBinContent(i)
        if isPrint:
            print "     %s, %s :  %s\n"%(x_l, x_u, SF_dic[(x_l,x_u)])
    if isPrint:print SF_dic
    return SF_dic

#main()
write_json(get_1D_json("NNPDF_weight_new.root","NNPDF30Over31",True),"Electron_NNPDF30Over31_new.txt")
#print "*"*50
