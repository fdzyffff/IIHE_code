...
/*        if(tmp_n_muon==2)
      ...
      for(unsigned int iEl=0 ; iEl<gsf_eta->size() ; ++iEl){
        ...
        if(isData)
        {
          TLorentzVector trigp4_dou25 ;
          el->match_trigger_l1_dou33 = true;

          for(unsigned int itrig=0 ; itrig<trig_dou25MW_unseeded_eta->size() ; ++itrig)
          { 
            trigp4_dou25.SetPtEtaPhiM(100,trig_dou25MW_unseeded_eta->at(itrig),trig_dou25MW_unseeded_phi->at(itrig),10) ;
            if(trigp4_dou25.DeltaR(el->p4_sc) < 0.1)
            { 
