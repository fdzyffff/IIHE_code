...
//    bool accept_missingHits = true ;
    ...
    isTag = (Et>35 && region==1 && accept_tag_ID && match_trigger_ele27 && match_trigger_dou33 && match_trigger_dou33L1 && L1Et > 40) ? 1 : 0 ;
    isFired = (accept_tag_ID && match_trigger_dou33) ? 1 : 0 ;
    isFired_unseeded = (accept_tag_ID && match_trigger_dou33_unseeded) ? 1 : 0 ;
    isEtPassed = (accept_tag_ID && match_trigger_dou33Et) ? 1 : 0 ;
    isEtPassed_unseeded = (accept_tag_ID && match_trigger_dou33Et_unseeded) ? 1 : 0 ;
    isL1Passed = (accept_tag_ID && match_trigger_dou33L1) ? 1 : 0 ;
...
//   std::vector<runCounter> runs ;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++){
      ...
      if(true==isData){
        if(ev_run<276453 || ev_run>278822) continue;
      }
      ...
      for(unsigned int iEl=0 ; iEl<gsf_n ; ++iEl){
        ...
        if(isData){
          Pass_DoubleEle33 = Ele33_trig_fire;
   
          vector<float>* trig_ele27_eta = trig_ele27_eta_v1  ;
          vector<float>* trig_ele27_phi = trig_ele27_phi_v1  ;

          TLorentzVector trigp4 ;

          for(unsigned int itrig=0 ; itrig<trig_ele27_eta->size() ; ++itrig){
            trigp4.SetPtEtaPhiM(100,trig_ele27_eta->at(itrig),trig_ele27_phi->at(itrig),10) ;
            if(trigp4.DeltaR(el->p4) < 0.1){
              el->match_trigger_ele27 = true ;
              break ;
            }
          }
          ...
          for(unsigned int itrig=0 ; itrig<trig_dou33_eta->size() ; ++itrig){
            ...
            if(trigp4_dou33.DeltaR(el->p4) < 0.1){
              el->match_trigger_dou33 = true ;
              break ;
            }
          }

          TLorentzVector trigp4_dou33L1 ;

          for(unsigned int itrig=0 ; itrig<trig_dou33L1_eta->size() ; ++itrig){
            trigp4_dou33L1.SetPtEtaPhiM(100,trig_dou33L1_eta->at(itrig),trig_dou33L1_phi->at(itrig),10) ;
            if(trigp4_dou33L1.DeltaR(el->p4) < 0.1){
              el->match_trigger_dou33L1 = true ;
              el->L1Et = trig_dou33L1_et->at(itrig);
              break ;
            }
          }

          TLorentzVector trigp4_dou33Et ;

          for(unsigned int itrig=0 ; itrig<trig_dou33Et_eta->size() ; ++itrig){
            trigp4_dou33Et.SetPtEtaPhiM(100,trig_dou33Et_eta->at(itrig),trig_dou33Et_phi->at(itrig),10) ;
            if(trigp4_dou33Et.DeltaR(el->p4) < 0.1){
              el->match_trigger_dou33Et = true ;
              break ;
            }
          }


          TLorentzVector trigp4_dou33_unseeded ;

          for(unsigned int itrig=0 ; itrig<trig_dou33_unseeded_eta->size() ; ++itrig){
            trigp4_dou33_unseeded.SetPtEtaPhiM(100,trig_dou33_unseeded_eta->at(itrig),trig_dou33_unseeded_phi->at(itrig),10) ;
            if(trigp4_dou33_unseeded.DeltaR(el->p4) < 0.1){
              el->match_trigger_dou33_unseeded = true ;
              break ;
            }
          }

          TLorentzVector trigp4_dou33Et_unseeded ;

          for(unsigned int itrig=0 ; itrig<trig_dou33Et_unseeded_eta->size() ; ++itrig){
            trigp4_dou33Et_unseeded.SetPtEtaPhiM(100,trig_dou33Et_unseeded_eta->at(itrig),trig_dou33Et_unseeded_phi->at(itrig),10) ;
            if(trigp4_dou33Et_unseeded.DeltaR(el->p4) < 0.1){
              el->match_trigger_dou33Et_unseeded = true ;
              break ;
            }
          }
      ...
      for(i=0;i<electrons.size();i++)
      {
         electron_candidate* probe = electrons.at(i) ;
         if(false==probe->isProbe) continue ;
         if(probe->region==3)
         {
            flg_probe = i;
            break;
